...
type Screen = "home" | "delivery" | "stores" | "store" | "productQuantity" | "storeOffers" | "offerQuantity" | "taxi" | "intercity" | "offers" | "checkout" | "driverSearch" | "orderTracking" | "account" | "customerOrders";
...
export default function Home() {
  ...
  return (
    <main dir="rtl" className="lahza-app-shell min-h-screen text-slate-950" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
      ...
      {screen === "home" ? (
        <>
          <div className="city-welcome-ticker" role="status"><div className="city-welcome-ticker-track"><span>{interfaceSettingsQuery.data?.tickerPrimary ?? "لم يفوتك أي جديد"}</span><span>اهلا بكم في مدينة {CITY_LABELS[selectedCity ?? "manbij"]}</span><span>{interfaceSettingsQuery.data?.tickerPrimary ?? "لم يفوتك أي جديد"}</span></div></div>
