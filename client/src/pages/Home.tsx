...
export default function Home() {
  ...
  const submitCheckout = () => {
    ...
    const hasAddress = deliveryAddress.trim().length >= 3;
    ...
    if (!isTaxi && !hasAddress) {
      toast.error("اكتب عنواناً واضحاً لتوصيل الطلب");
      return;
    }
    ...
    createOrder.mutate({
      ...
      locationText: isTaxi ? (customerLocation.trim() || pickup.trim()) : deliveryAddress.trim(),
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-white text-slate-950">
      ...
      {screen !== "store" && screen !== "productQuantity" && screen !== "storeOffers" ? <div className="global-offer-bar"><PartnerOfferGallery slides={partnerGallerySlides} onOpen={setSelectedGalleryOffer} /></div> : null}
...
function CartPreview({ cart, removeLine, total, hasPharmacy, deliveryFeeNewSyp, deliveryPercent, deliveryArea, grandTotalNewSyp, onContinueShopping }: { cart: CartLine[]; removeLine: (id: string) => void; total: number; hasPharmacy: boolean; deliveryFeeNewSyp: number; deliveryPercent: number; deliveryArea: "منبج" | "جرابلس"; grandTotalNewSyp: number; onContinueShopping: () => void }) {
  return <div className="cart-preview"><div className="cart-preview-intro"><span className="cart-preview-icon"><ShoppingBasket className="h-5 w-5" /></span><div><strong>اختياراتك في لحظة</strong><p>{cart.length} {cart.length === 1 ? "صنف مضاف" : "أصناف مضافة"} إلى السلة</p></div><button type="button" onClick={onContinueShopping}>إضافة منتجات</button></div><div className="cart-lines">{cart.map(item => <div key={item.id} className="cart-line"><span className="cart-line-category" aria-hidden="true">{homeCategoryEmoji[item.category] ?? "🛍️"}</span><div className="cart-line-copy"><strong>{item.itemName}</strong><span>{item.quantity} {item.unit}</span></div><div className="cart-line-actions">{item.priceKnown ? <strong>{formatSyp(lineTotal(item))}</strong> : <small>السعر عند التأكيد</small>}<button type="button" onClick={() => removeLine(item.id)} aria-label={`حذف ${item.itemName}`}><Trash2 className="h-4 w-4" /></button></div></div>)}</div><p className="pharmacy-note"><Pill className="h-4 w-4" />{hasPharmacy ? "تتضمن السلة منتجات صحية؛ جميع الأسعار تقديرية ويؤكدها المتجر قبل التجهيز." : "الأسعار المعروضة تقديرية وقابلة للتعديل من المتجر قبل تأكيد الطلب."}</p><div className="cart-totals"><div className="total-row"><span>إجمالي المنتجات التقديري</span><strong>{formatSyp(total)}</strong></div><div className="total-row"><span>رسوم توصيل {deliveryArea} ({deliveryPercent}%)</span><strong>{formatNewSyp(deliveryFeeNewSyp)}</strong></div><div className="total-row total-row-grand"><span>الإجمالي النهائي</span><strong>{formatNewSyp(grandTotalNewSyp)}</strong></div></div><p className="cart-minimum-note">يُحسب الحد الأدنى 300 ل.س جديدة من قيمة المنتجات فقط، قبل رسوم التوصيل.</p></div>;
}
