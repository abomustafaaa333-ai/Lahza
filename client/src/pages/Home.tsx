...
const homeFeaturedStores: { name: string; category: LahzaCategory; note: string; ratingStars: number }[] = [
  { name: "مذاق الساحة", category: "restaurants", note: "وجبات ومشاوي", ratingStars: 3 },
  { name: "سوق الندى", category: "groceries", note: "مؤونة واحتياجات البيت", ratingStars: 3 },
  { name: "أفران الصباح", category: "bakery", note: "خبز ومعجنات طازجة", ratingStars: 3 },
  { name: "حلويات السعادة", category: "sweets", note: "ضيافة وحلويات شامية", ratingStars: 3 },
];
...
function ServiceIntroCarousel({ onActiveChange, onExplore }: { onActiveChange: (index: number) => void; onExplore: () => void }) {
  const slides = [
  ...
  return <section className="service-intro-panel" aria-label="خدمات لحظة"><div className="service-intro-copy"><span className="service-intro-kicker">خدمات لحظة</span><strong>{slide.title}</strong><small>{slide.detail}</small><button type="button" className="service-intro-cta" onClick={onExplore}>استكشف الآن <ChevronLeft className="h-4 w-4" /></button></div><span className="service-intro-icon service-intro-basket" aria-hidden="true"><ShoppingBasket className="h-7 w-7" /></span><div className="service-intro-dots">{slides.map((item, index) => <button key={item.title} type="button" className={index === active ? "service-intro-dot-active" : ""} onClick={() => changeActive(index)} aria-label={`الشريحة ${index + 1}`} />)}</div></section>;
}
...
export default function Home() {
  ...
  const featuredStoreCards = useMemo(() => {
    ...
    const uniqueStores = new Map<number, { name: string; category: LahzaCategory; note: string; ratingStars: number }>();
    for (const offer of partnerOffers) {
      const storeId = Number(offer.storeId ?? 0);
      const category = offer.storeCategory as LahzaCategory;
      if (storeId > 0 && !uniqueStores.has(storeId) && categoryImageByKey[category]) uniqueStores.set(storeId, { name: offer.storeName || "متجر مميز", category, note: "لديه عروض مميزة نشطة", ratingStars: offer.ratingStars ?? 3 });
    }
      ...
      {screen === "home" ? (
        <>
          <section className="app-shell pb-10">
            ...
            <section className="home-showcase-block"><div className="home-section-heading"><div><p className="section-eyebrow">اختيارات لحظة</p><h2 className="section-title">المتاجر المميزة</h2></div><button type="button" onClick={() => setScreen("delivery")}>عرض الكل <ChevronLeft className="h-4 w-4" /></button></div><div className="home-featured-row">{featuredStoreCards.length ? featuredStoreCards.map(store => <button key={store.name} type="button" className="home-featured-store" onClick={() => { setActiveCategory(store.category); setActiveCustomCategory(null); setSelectedStore(null); setScreen("stores"); }}><img src={categoryImageByKey[store.category]} alt="" loading="lazy" /><span className="home-store-icon"><LahzaCategoryIcon category={store.category} className="h-4 w-4" /></span><strong>{store.name}</strong><span className="home-store-rating" aria-label={`تقييم ${store.ratingStars ?? 3} من 5`}>{"★".repeat(store.ratingStars ?? 3)}{"☆".repeat(5 - (store.ratingStars ?? 3))}</span><small>{store.note}</small><span className="home-store-open">لديه عروض</span></button>) : <div className="home-featured-empty">لا توجد متاجر لديها عروض مميزة حالياً</div>}</div></section>
