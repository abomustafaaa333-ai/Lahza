...
function CustomerAuthScreen({ onAuthenticated, onSecret }: { onAuthenticated: (session: CustomerAuthSession) => void; onSecret: (phone?: string, role?: "owner" | "supervisor" | "partner") => void }) {
  ...
  const loginDirectly = async () => {
    ...
    if (staffResult.data) { onSecret(fullPhone, staffResult.data.role); return; }
...
export default function Home() {
  ...
  if (!customerAuth && !hasStaffSession) return <><CustomerAuthScreen onAuthenticated={completeCustomerAuth} onSecret={(phone, role) => { if (phone) setUsername(phone); if (role) setSecretRole(role); setSecretOpen(true); }} />{adminAccessDialog}</>;
