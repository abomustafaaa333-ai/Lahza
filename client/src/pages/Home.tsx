...
export default function Home() {
  ...
  const openOfferLocation = (offer: PartnerGallerySlide) => {
    ...
    setFocusedOfferId(offer.id > 0 ? offer.id : null);
    setScreen("offers");
  };
  ...
  return (
    <main dir="rtl" className="lahza-app-shell min-h-screen text-slate-950">
      ...
      {screen !== "store" && screen !== "productQuantity" && screen !== "storeOffers" && screen !== "checkout" ? <div className="global-offer-bar"><PartnerOfferGallery slides={partnerGallerySlides} onOpen={openOfferLocation} /></div> : null}
...
function CustomerOfferArtwork({ offer, compact = false }: { offer: CustomerOffer; compact?: boolean }) {
  ...
  return <div className={`lahza-offer-artwork ${compact ? "lahza-offer-artwork-compact" : ""}`}><div className="lahza-offer-artwork-glow" /><span className="lahza-offer-artwork-brand">لحظة</span><span className="lahza-offer-artwork-badge">خصم {offer.discountPercent || "—"}%</span><div className="lahza-offer-artwork-copy"><small>{offer.storeName ?? offer.partnerName}</small><strong>{offer.text}</strong><span>{offer.productName ?? "عرض مميز"}</span><div className="lahza-offer-artwork-prices">{originalPrice > 0 ? <span className="lahza-offer-price-old"><i>قبل العرض</i><del>{formatSyp(originalPrice)}</del></span> : null}{discountedPrice > 0 ? <span className="lahza-offer-price-new"><i>سعر العرض</i><b>{formatSyp(discountedPrice)}</b></span> : null}</div></div><div className="lahza-offer-artwork-product">{productImage ? <img src={productImage} alt={offer.productName ?? offer.text} /> : <BadgePercent className="h-12 w-12" />}</div></div>;
}
