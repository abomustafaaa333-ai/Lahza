...
import { ArrowLeft, BadgePercent, BellRing, Bike, CakeSlice, CarFront, CheckCircle2, ChevronLeft, CircleHelp, ClipboardList, Clock3, CreditCard, Download, Fuel, HandCoins, ImagePlus, LayoutDashboard, Loader2, LocateFixed, LogOut, MapPinCheck, MessageCircle, Minus, PackageCheck, PackagePlus, Pencil, Phone, Pill, Plus, QrCode, Search, Share2, Shirt, ShoppingBasket, Smartphone, Sparkles, Store, Trash2, Truck, UserRound, UtensilsCrossed, Wheat, X } from "lucide-react";
...
type PromotionPreview = { code: string; kind: "discount" | "referral" | "automatic"; percent: number; discountAmount: number; itemsTotal: number };
...
type StoreProduct = { id: number; name: string; unit: string; unitPrice: number; available: boolean; imageUrl?: string | null; imageUrls?: string | null; clothingSizes?: string | null; clothingColors?: string | null; clothingVariantImages?: string | null };
...
type LahzaEventSettings = { eventImageUrl?: string; eventLinkUrl?: string; eventEnabled?: boolean; eventStayVisible?: boolean; eventShowSeconds?: number; eventRepeatMinutes?: number };
...
function ServiceIntroCarousel({ onActiveChange, onExplore, onOffers, onGateway, city }: { onActiveChange: (index: number) => void; onExplore: () => void; onOffers: () => void; onGateway: () => void; city: CityKey }) {
  const slides = [
  ...
  return <section className="service-intro-panel" aria-label="خدمات لحظة">{city === "jarabulus" ? <button type="button" className="service-intro-manbij-cta" onClick={onGateway} aria-label="اطلب من منبج"><span className="service-intro-manbij-icon"><PackageCheck className="h-5 w-5" /></span><strong>اطلب من منبج</strong><ChevronLeft className="h-4 w-4" /></button> : null}<div className="service-intro-copy"><span className="service-intro-kicker">خدمات لحظة</span><span className="service-intro-tagline">{CITY_LABELS[city]} بين يديك</span><strong>{slide.title}</strong><small>{slide.detail}</small><div className="service-intro-actions"><button type="button" className="service-intro-cta" onClick={onExplore}>اكتشف خدمات لحظة <ChevronLeft className="h-4 w-4" /></button><button type="button" className="service-intro-offers-cta" onClick={onOffers}>شاهد العروض <BadgePercent className="h-4 w-4" /></button></div></div><div className="service-intro-dots">{slides.map((item, index) => <button key={item.title} type="button" className={index === active ? "service-intro-dot-active" : ""} onClick={() => changeActive(index)} aria-label={`الشريحة ${index + 1}`} />)}</div></section>;
}
...
function LahzaEventCover({ event }: { event?: LahzaEventSettings }) {
  const [visible, setVisible] = useState(false);
  useEffect(() => {
    if (!event?.eventEnabled || !event.eventImageUrl) { setVisible(false); return; }
    let hideTimer: number | undefined;
    let repeatTimer: number | undefined;
    const show = () => { setVisible(true); if (!event.eventStayVisible) hideTimer = window.setTimeout(() => setVisible(false), (event.eventShowSeconds ?? 5) * 1000); };
    const initialTimer = window.setTimeout(show, 1000);
    if ((event.eventRepeatMinutes ?? 0) > 0) repeatTimer = window.setInterval(show, (event.eventRepeatMinutes ?? 0) * 60 * 1000);
    return () => { window.clearTimeout(initialTimer); if (hideTimer) window.clearTimeout(hideTimer); if (repeatTimer) window.clearInterval(repeatTimer); };
  }, [event?.eventEnabled, event?.eventImageUrl, event?.eventStayVisible, event?.eventShowSeconds, event?.eventRepeatMinutes]);
  if (!visible || !event?.eventImageUrl) return null;
  const image = <img src={event.eventImageUrl} alt="إعلان أو مسابقة لحظة" className="lahza-event-cover-image" />;
  return <div className="lahza-event-cover" role="dialog" aria-label="إعلان لحظة">{event.eventLinkUrl ? <a href={event.eventLinkUrl} target="_blank" rel="noreferrer" className="lahza-event-cover-link">{image}</a> : image}<button type="button" onClick={() => setVisible(false)} className="lahza-event-cover-close" aria-label="إغلاق الإعلان">×</button></div>;
}

function Header({ onSearch, onExplore, onOffers, onGateway, searchPlaceholder, city, event }: { onSearch: () => void; onExplore: () => void; onOffers: () => void; onGateway: () => void; searchPlaceholder: string; city: CityKey; event?: LahzaEventSettings }) {
...
function CustomerAuthScreen({ onAuthenticated, onStaffLogin }: { onAuthenticated: (session: CustomerAuthSession) => void; onStaffLogin: (phone: string, role: "owner" | "supervisor" | "partner" | "driver", password: string) => void }) {
  ...
  const loginDirectly = async () => {
    ...
    setError("");
    try {
      const result = await staffLookupQuery.refetch();
      ...
      setError(lookupError instanceof Error ? lookupError.message : "تعذر التحقق من الحساب حالياً. حاول مرة أخرى.");
    }
  };
...
export default function Home() {
  ...
  const [automaticDiscountPreview, setAutomaticDiscountPreview] = useState<PromotionPreview | null>(null);
  ...
  const [pharmacyPrescriptionDataUrl, setPharmacyPrescriptionDataUrl] = useState("");
  const [pharmacyPrescriptionUrl, setPharmacyPrescriptionUrl] = useState("");
  ...
  const handleTouchEnd = () => {
    ...
    const refreshQueries = [
      utils.lahza.interfaceSettings.get.invalidate(),
      utils.lahza.customCategories.listActive.invalidate(),
      utils.lahza.publicFeaturedOffers.invalidate(),
      utils.lahza.storefront.stores.invalidate(),
      utils.lahza.storefront.gatewayStores.invalidate(),
      utils.lahza.storefront.products.invalidate(),
    ];
    void Promise.all(refreshQueries).then(() => toast.success("تم تحديث البيانات")).catch(() => toast.error("تعذر إكمال التحديث، حاول مرة أخرى")).finally(() => window.setTimeout(() => setRefreshing(false), 450));
  };
  ...
  const previewAutomaticDiscount = trpc.lahza.orders.automaticDiscount.useMutation();
  const uploadPharmacyPrescription = trpc.lahza.orders.uploadPrescription.useMutation();
  const createOrder = trpc.lahza.orders.create.useMutation({
    onSuccess: result => {
      toast.success(`تم إرسال الطلب رقم #${result.orderId} بنجاح`);
      setPharmacyPrescriptionDataUrl("");
      setPharmacyPrescriptionUrl("");
  ...
  const activePromotion = discountPreview ?? referralPreview ?? automaticDiscountPreview;
  ...
  useEffect(() => {
    if (isStaticDemo || !cartOrderLines.length) { setAutomaticDiscountPreview(null); return; }
    let active = true;
    void previewAutomaticDiscount.mutateAsync({ lines: cartOrderLines }).then(result => { if (active) setAutomaticDiscountPreview(result); }).catch(() => { if (active) setAutomaticDiscountPreview(null); });
    return () => { active = false; };
  }, [cart, isStaticDemo]);
  ...
  const submitCheckout = async (confirmedDeliveryFee = false) => {
    ...
    if (!isTaxi && !isWosselLi && !hasPharmacy && toNewSyp(total) < minimumDeliveryOrderSyp) {
    ...
    if (hasPharmacy && !pharmacyPrescriptionDataUrl && !pharmacyPrescriptionUrl) {
      toast.error("أرفق صورة الوصفة الطبية قبل إرسال طلب الصيدلية");
      return;
    }
    if (isStaticDemo) {
    ...
    let resolvedPharmacyPrescriptionUrl = pharmacyPrescriptionUrl || undefined;
    if (hasPharmacy && pharmacyPrescriptionDataUrl) {
      try {
        const uploaded = await uploadPharmacyPrescription.mutateAsync({ dataUrl: pharmacyPrescriptionDataUrl });
        resolvedPharmacyPrescriptionUrl = uploaded.imageUrl;
        setPharmacyPrescriptionUrl(uploaded.imageUrl);
      } catch (error) {
        toast.error(error instanceof Error ? error.message : "تعذر رفع صورة الوصفة");
        return;
      }
    }
    createOrder.mutate({
      ...
      pharmacyPrescriptionUrl: hasPharmacy ? resolvedPharmacyPrescriptionUrl : undefined,
  ...
  const driverAppMode = new URLSearchParams(window.location.search).get("driverApp") === "1";
  if (driverSessionQuery.data && !driverAppMode) return <DriverDashboard session={driverSessionQuery.data} onLogout={() => driverLogout.mutate()} />;
  ...
  return (
    <main dir="rtl" className="lahza-app-shell min-h-screen text-slate-950" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
      ...
      <div className="top-action-cluster" dir="ltr">{driverSessionQuery.data ? <button type="button" className="persistent-notifications-button" onClick={() => window.location.href = "/?driverApp=1"} aria-label="فتح لوحة المندوب"><LayoutDashboard className="h-5 w-5" /></button> : null}<PersistentCartButton onCart={openCart} cartCount={cart.length} /><button type="button" className="persistent-notifications-button" onClick={() => setNotificationsOpen(true)} aria-label="فتح الإشعارات"><BellRing className="h-5 w-5" />{((notificationsQuery.data ?? []).filter(notification => notification.unread).length + (orderNotificationsQuery.data ?? []).length) > 0 ? <b>{(notificationsQuery.data ?? []).filter(notification => notification.unread).length + (orderNotificationsQuery.data ?? []).length}</b> : null}</button></div><LahzaEventCover event={interfaceSettingsQuery.data} />
      {screen === "home" ? <Header onSearch={() => setSearchOpen(true)} onExplore={() => setServiceDirectoryOpen(true)} onOffers={openFeaturedOffers} onGateway={() => { setGatewayMode(true); setScreen("stores"); }} searchPlaceholder={searchPlaceholder} city={selectedCity ?? "manbij"} event={interfaceSettingsQuery.data} /> : null}
      ...
      {screen === "home" ? (
        <>
          ...
          <section className="app-shell pb-10 home-discover-section">
            ...
            <section className="home-showcase-block"><div className="home-section-heading"><div><h2 className="section-title">المتاجر المميزة</h2></div><button type="button" onClick={() => setScreen("delivery")}>عرض الكل <ChevronLeft className="h-4 w-4" /></button></div><div className="home-featured-row">{featuredStoreCards.length ? featuredStoreCards.map(store => <button key={`${store.id}-${store.name}`} type="button" className="home-featured-store" onClick={() => { setActiveCategory(store.category); setActiveCustomCategory(null); setSelectedProduct(null); setSelectedOffer(null); setSelectedStore({ id: Number(store.id), name: store.name, category: store.category, storeOpen: store.storeOpen }); setScreen("store"); window.scrollTo({ top: 0, behavior: "smooth" }); }}><img src={categoryImageByKey[store.category]} alt="" loading="lazy" /><strong>{store.name}</strong><span className="home-store-rating" aria-label={`تقييم ${store.ratingStars ?? 3} من 5`}>{"★".repeat(store.ratingStars ?? 3)}{"☆".repeat(5 - (store.ratingStars ?? 3))}</span><small>{store.note}</small><span className="home-store-open">لديه عروض</span></button>) : <div className="home-featured-empty">لا توجد متاجر لديها عروض مميزة حالياً</div>}</div></section>
      ...
      {screen === "productQuantity" && selectedStore && selectedProduct ? <ProductQuantityScreen store={selectedStore} product={selectedProduct} storeOpen={selectedStore.storeOpen} onBack={() => setScreen("store")} onAdd={({ quantity, size, color }) => { void addFromStore(selectedStore.id, () => addLine({ category: selectedStore.category, itemName: `${selectedProduct.name}${size ? ` · المقاس ${size}` : ""}${color ? ` · اللون ${color}` : ""}`, quantity, unit: selectedProduct.unit, unitPrice: selectedProduct.unitPrice, catalogItemId: selectedProduct.id < 0 ? undefined : selectedProduct.id, priceKnown: selectedProduct.unitPrice > 0 }, "store")); }} /> : null}
...
function CartPreview({ cart, removeLine, updateQuantity, total, hasPharmacy, deliveryFeeNewSyp, deliveryArea, grandTotalNewSyp, deliveryQuoteReady, onContinueShopping }: { cart: CartLine[]; removeLine: (id: string) => void; updateQuantity: (id: string, quantity: number) => void; total: number; hasPharmacy: boolean; deliveryFeeNewSyp: number; deliveryArea: "منبج" | "جرابلس"; grandTotalNewSyp: number; deliveryQuoteReady: boolean; onContinueShopping: () => void }) {
...
type DriverDashboardSession = { name: string; phone: string; available: boolean; driverPercent: number; completedOrdersTotal: number; assignments: Array<{ orderId: number; assignmentStatus: string; orderStatus: string; customerName: string; customerPhone: string; locationText: string | null; createdAt: string | Date }>; completedOrders: Array<{ orderId: number; customerName: string; totalAmount: number; deliveryFee: number; driverFee: number; deliveredAt: string | Date | null }> };
function DriverDashboard({ session, onLogout }: { session: DriverDashboardSession; onLogout: () => void }) {
  ...
  const setAvailable = trpc.lahza.driverAuth.setAvailable.useMutation({ onSuccess: () => void driverSessionQuery.refetch(), onError: error => toast.error(error.message) });
  const respondToAssignment = trpc.lahza.driverAuth.respondToAssignment.useMutation({ onSuccess: () => void driverSessionQuery.refetch(), onError: error => toast.error(error.message) });
  ...
  const available = driverSessionQuery.data?.available ?? session.available;
  return <main dir="rtl" className="min-h-screen bg-[#fffaf6] px-4 py-8 text-[#4a2618]"><section className="mx-auto max-w-lg space-y-5"><div className="rounded-[2rem] bg-[#63301b] p-6 text-white shadow-xl"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold text-orange-100">لحظة · لوحة المندوب</p><h1 className="mt-2 text-2xl font-black">مرحباً {session.name}</h1><p className="mt-2 text-sm text-orange-100" dir="ltr">{session.phone}</p></div><button type="button" onClick={() => window.location.href = "/"} className="rounded-xl bg-white/15 px-3 py-2 text-xs font-black text-white">تطبيق لحظة</button></div><div className="mt-5 flex items-center justify-between gap-3 rounded-2xl bg-white/10 p-3"><span className="text-sm font-black">حالة العمل: {available ? "متاح للعمل" : "غير متاح"}</span><button type="button" disabled={setAvailable.isPending} onClick={() => setAvailable.mutate({ available: !available })} className={`rounded-xl px-3 py-2 text-xs font-black ${available ? "bg-emerald-400 text-emerald-950" : "bg-white text-[#63301b]"}`}>{available ? "متاح للعمل" : "غير متاح"}</button></div><div className="mt-4 rounded-2xl bg-white/10 p-4"><p className="text-xs text-orange-100">جرد عمولتك اليوم</p><div className="mt-2 flex items-end justify-between gap-3"><strong className="text-3xl font-black">{formatSyp(completedOrdersTotal)}</strong><span className="text-sm font-bold text-orange-100">نسبة العمولة {driverPercent}%</span></div></div></div><div className="grid grid-cols-2 gap-3"><button type="button" onClick={() => setView("active")} className={`rounded-2xl px-4 py-3 text-sm font-black ${view === "active" ? "bg-[#63301b] text-white" : "border border-orange-100 bg-white text-slate-500"}`}>الطلبات الحالية</button><button type="button" onClick={() => setView("completed")} className={`rounded-2xl px-4 py-3 text-sm font-black ${view === "completed" ? "bg-emerald-700 text-white" : "border border-orange-100 bg-white text-slate-500"}`}>طلبات اليوم المكتملة ({completedOrders.length})</button></div><section className="rounded-3xl border border-orange-100 bg-white p-5 shadow-sm">{view === "active" ? assignments.length ? <div className="space-y-3">{assignments.map(order => <article key={order.orderId} className="rounded-2xl bg-slate-50 p-4"><div className="flex items-center justify-between gap-3"><strong>طلب #{order.orderId}</strong><span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-teal-700">{order.assignmentStatus === "accepted" ? "مقبول" : order.assignmentStatus === "picked_up" ? "تم الاستلام" : "طلب جديد"}</span></div><p className="mt-2 text-sm font-bold text-slate-600">العميل: {order.customerName}</p><p className="mt-1 text-sm font-bold text-slate-700" dir="ltr">{order.customerPhone}</p><p className="mt-1 text-xs leading-6 text-slate-500">{order.locationText || "موقع التسليم محدد"}</p><div className="mt-3 flex flex-wrap gap-2"><a href={`tel:${order.customerPhone}`} className="inline-flex flex-1 items-center justify-center gap-1 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white"><Phone className="h-4 w-4" /> اتصال</a><a href={`https://wa.me/${order.customerPhone.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" className="inline-flex flex-1 items-center justify-center gap-1 rounded-xl bg-teal-600 px-3 py-2 text-xs font-black text-white"><MessageCircle className="h-4 w-4" /> محادثة</a>{order.assignmentStatus === "assigned" ? <><button type="button" disabled={respondToAssignment.isPending} onClick={() => respondToAssignment.mutate({ orderId: order.orderId, accept: true })} className="flex-1 rounded-xl bg-[#63301b] px-3 py-2 text-xs font-black text-white">قبول الطلب</button><button type="button" disabled={respondToAssignment.isPending} onClick={() => respondToAssignment.mutate({ orderId: order.orderId, accept: false })} className="flex-1 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-black text-red-700">عدم قبول</button></> : null}</div></article>)}</div> : <p className="text-center text-sm font-bold leading-7 text-slate-500">لا توجد طلبات نشطة حالياً.</p> : completedOrders.length ? <div className="space-y-3">{completedOrders.map(order => <article key={order.orderId} className="rounded-2xl bg-emerald-50 p-4"><div className="flex items-center justify-between gap-3"><strong>طلب #{order.orderId}</strong><strong className="text-emerald-700">عمولتك {formatSyp(order.driverFee)}</strong></div><p className="mt-2 text-sm font-bold text-slate-600">العميل: {order.customerName}</p><div className="mt-2 flex justify-between text-xs text-slate-500"><span>رسوم التوصيل: {formatSyp(order.deliveryFee)}</span><span>{order.deliveredAt ? new Date(order.deliveredAt).toLocaleTimeString("ar-SY", { hour: "2-digit", minute: "2-digit" }) : "اليوم"}</span></div></article>)}</div> : <p className="text-center text-sm font-bold leading-7 text-slate-500">لم تُكمل أي طلب اليوم بعد.</p>}</section><button type="button" onClick={onLogout} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-rose-200 bg-white px-5 py-3 text-sm font-black text-[#63301b]"><LogOut className="h-4 w-4" /> تسجيل الخروج</button></section></main>;
}
