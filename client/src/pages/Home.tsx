...
function ServiceIntroCarousel({ onActiveChange, onExplore }: { onActiveChange: (index: number) => void; onExplore: () => void }) {
  ...
  return <section className="service-intro-panel" aria-label="خدمات لحظة"><div className="service-intro-copy"><span className="service-intro-kicker">خدمات لحظة</span><strong>{slide.title}</strong><small>{slide.detail}</small><button type="button" className="service-intro-cta" onClick={onExplore}>استكشف الآن <ChevronLeft className="h-4 w-4" /></button></div><span className="service-intro-icon service-intro-basket" aria-hidden="true"><img src="/assets/lahza-hero-shopping-basket.jpg" alt="سلة تسوق مليئة بالخضروات والمواد" /></span><div className="service-intro-dots">{slides.map((item, index) => <button key={item.title} type="button" className={index === active ? "service-intro-dot-active" : ""} onClick={() => changeActive(index)} aria-label={`الشريحة ${index + 1}`} />)}</div></section>;
}
      ...
      {screen === "home" ? (
        <>
          <section className="app-shell pb-10">
            ...
            <section className="home-showcase-block"><div className="home-section-heading"><div><p className="section-eyebrow">اختيارات لحظة</p><h2 className="section-title">المتاجر المميزة</h2></div><button type="button" onClick={() => setScreen("delivery")}>عرض الكل <ChevronLeft className="h-4 w-4" /></button></div><div className="home-featured-row">{featuredStoreCards.length ? featuredStoreCards.map(store => <button key={store.name} type="button" className="home-featured-store" onClick={() => { setActiveCategory(store.category); setActiveCustomCategory(null); setSelectedStore(null); setScreen("stores"); }}><img src={categoryImageByKey[store.category]} alt="" loading="lazy" /><strong>{store.name}</strong><span className="home-store-rating" aria-label={`تقييم ${store.ratingStars ?? 3} من 5`}>{"★".repeat(store.ratingStars ?? 3)}{"☆".repeat(5 - (store.ratingStars ?? 3))}</span><small>{store.note}</small><span className="home-store-open">لديه عروض</span></button>) : <div className="home-featured-empty">لا توجد متاجر لديها عروض مميزة حالياً</div>}</div></section>
