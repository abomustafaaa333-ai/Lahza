  ...
  return (
    <header className="sticky top-0 z-30 border-b border-[#ff6b2d] bg-[#ff7a33]/95 pt-3 backdrop-blur-xl">
      <div className="app-shell flex h-[76px] items-center justify-end gap-3">
        <button className="brand-mark" onDoubleClick={onSecret} title="انقر مرتين لدخول المالك أو المشرف أو الشريك" aria-label="لحظة — خدمات توصيل منبج وريفها"><span className="lahza-logo-lockup"><span className="lahza-logo-symbol" aria-hidden="true"><i /></span><span className="lahza-logo-word">لحظة</span></span><small>كل شيء في لحظة</small><span className="brand-location-label" aria-label="خدمات توصيل منبج وريفها"><span>خدمات</span><span>توصيل</span><span>منبج</span><span>وريفها</span></span></button>
...
function PartnerOfferGallery({ slides, onOpen }: { slides: PartnerGallerySlide[]; onOpen: () => void }) {
  ...
  return <div className="hero-image-frame" aria-label="عروض مصورة من متاجر لحظة">{slides.length ? <><div className="hero-gallery-track" style={{ width: `${slides.length * 100}%`, transform: `translateX(-${activeIndex * (100 / slides.length)}%)` }}>{slides.map((slide, slideIndex) => <button key={slide.id} type="button" onClick={onOpen} style={{ flex: `0 0 ${100 / slides.length}%`, minWidth: `${100 / slides.length}%`, backgroundImage: `url("${slide.imageUrl}"), url("/assets/lahza-offer-${slideIndex % 3 === 0 ? "bakery" : slideIndex % 3 === 1 ? "grocery" : "restaurant"}.jpg")`, backgroundSize: "cover", backgroundPosition: "center" }} className="hero-gallery-slide block cursor-pointer text-right" aria-label={`فتح العروض المميزة: ${slide.name}`}><img src={slide.imageUrl} alt={`عرض ${slide.name} من ${slide.partnerName}`} onError={event => { const fallbacks = ["/assets/lahza-offer-bakery.jpg", "/assets/lahza-offer-grocery.jpg", "/assets/lahza-offer-restaurant.jpg"]; const fallback = slide.fallbackImageUrl || fallbacks[slideIndex % fallbacks.length]; if (event.currentTarget.dataset.fallbackApplied !== "true") { event.currentTarget.dataset.fallbackApplied = "true"; event.currentTarget.src = fallback; } }} className="hero-offer-image" />{slide.discountPercent ? <span className="hero-offer-discount">خصم {slide.discountPercent}%</span> : null}<div className="hero-offer-caption"><strong>{slide.name}</strong><span>{slide.partnerName}</span>{slide.originalProductPrice || slide.offerPrice || slide.unitPrice ? <span className="hero-offer-prices"><del>{formatSyp(slide.originalProductPrice || slide.unitPrice)}</del><b>فقط {formatSyp(slide.offerPrice || slide.unitPrice)}</b></span> : null}</div></button>)}</div>{slides.length > 1 ? <div className="hero-gallery-dots" aria-label="صور العروض">{slides.map((slide, index) => <button key={slide.id} type="button" onClick={() => setActiveIndex(index)} aria-label={`عرض الصورة ${index + 1}`} aria-current={index === activeIndex} className={index === activeIndex ? "hero-gallery-dot-active" : ""} />)}</div> : null}</> : <div className="hero-gallery-empty"><BadgePercent className="h-9 w-9" /><strong>عروض لحظة</strong><span>ترقّبوا أحدث العروض من متاجركم المفضلة</span><button type="button" onClick={onOpen} className="hero-gallery-cta">اكتشف العروض</button></div>}</div>;
}
export default function Home() {
  ...
  return (
    <main dir="rtl" className="lahza-app-shell min-h-screen text-slate-950">
      ...
      {screen !== "store" && screen !== "productQuantity" && screen !== "storeOffers" && screen !== "checkout" ? <div className="global-offer-bar"><PartnerOfferGallery slides={partnerGallerySlides} onOpen={openFeaturedOffers} /></div> : null}
      ...
      {screen === "home" ? (
        <>
          ...
          <section className="app-shell pb-10">
            ...
            <section className="home-showcase-block"><div className="home-section-heading"><div><p className="section-eyebrow">اختيارات لحظة</p><h2 className="section-title">المتاجر المميزة</h2></div><button type="button" onClick={() => setScreen("delivery")}>عرض الكل <ChevronLeft className="h-4 w-4" /></button></div><div className="home-featured-row">{homeFeaturedStores.map(store => <button key={store.name} type="button" className="home-featured-store" onClick={() => { setActiveCategory(store.category); setActiveCustomCategory(null); setSelectedStore(null); setScreen("stores"); }}><img src={categoryImageByKey[store.category]} alt="" loading="lazy" /><span className="home-store-icon"><LahzaCategoryIcon category={store.category} className="h-4 w-4" /></span><strong>{store.name}</strong><small>{store.note}</small><span className="home-store-open">متاح الآن</span></button>)}</div></section>
