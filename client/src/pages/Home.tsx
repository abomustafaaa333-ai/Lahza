...
function SupportContactsDialog({ open, onOpenChange, contacts }: { open: boolean; onOpenChange: (open: boolean) => void; contacts: SupportContact[] }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent dir="rtl" className="w-[calc(100%-1.5rem)] max-w-md rounded-3xl border-0 bg-white p-5 shadow-2xl"><DialogHeader><DialogTitle className="flex items-center gap-2 text-right text-xl text-[#54162a]"><Phone className="h-5 w-5 text-[#ed665e]" /> تواصل معنا</DialogTitle><DialogDescription className="text-right">اختر الطريقة المناسبة، وسيصل اتصالك مباشرة إلى فريق لحظة.</DialogDescription></DialogHeader><div className="mt-3 space-y-3">{contacts.length ? contacts.map(contact => <article key={contact.id} className="rounded-2xl border border-rose-100 bg-[#fffaf8] p-4"><strong className="block text-sm text-[#54162a]">{contact.label}</strong><span dir="ltr" className="mt-1 block text-xs font-bold text-slate-500">{contact.phone}</span><div className="mt-3 flex flex-wrap gap-2">{contact.callEnabled ? <a href={`tel:${contact.phone}`} className="inline-flex items-center gap-2 rounded-xl bg-[#6f1028] px-3 py-2 text-xs font-black text-white transition hover:bg-[#54162a]"><Phone className="h-4 w-4" /> اتصال</a> : null}{contact.whatsappEnabled ? <a href={supportWhatsAppUrl(contact.phone)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white transition hover:bg-emerald-700"><MessageCircle className="h-4 w-4" /> واتساب</a> : null}</div></article>) : <div className="rounded-2xl border border-dashed border-rose-200 bg-rose-50 p-4 text-center text-sm font-bold leading-7 text-[#6f1028]">لا توجد أرقام تواصل منشورة حالياً. سيضيفها مدير لحظة من لوحة التحكم قريباً.</div>}</div></DialogContent></Dialog>;
}

function AboutLahzaDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent dir="rtl" className="w-[calc(100%-1.5rem)] max-w-md rounded-3xl border-0 bg-white p-5 shadow-2xl"><DialogHeader><DialogTitle className="flex items-center gap-2 text-right text-xl text-[#54162a]"><CircleHelp className="h-5 w-5 text-[#ed665e]" /> حول التطبيق</DialogTitle><DialogDescription className="text-right">لحظة — خدمات توصيل منبج.</DialogDescription></DialogHeader><div className="mt-3 rounded-2xl bg-[#fff6f1] p-4 text-sm leading-8 text-slate-600"><strong className="block text-base text-[#6f1028]">كل ما تحتاجه في لحظة</strong><p className="mt-2">يساعدك تطبيق لحظة على استكشاف المتاجر المحلية وطلب المنتجات، مع متابعة حالة الطلب والتواصل السهل مع فريق الخدمة عند الحاجة.</p><p className="mt-2">تغطي الخدمة حالياً مدينة منبج، ويجري تطوير المزيد من الخدمات تدريجياً.</p></div></DialogContent></Dialog>;
}

function SupportHelpCard({ onOpen, contactCount }: { onOpen: () => void; contactCount: number }) {
  return <div className="flex items-center gap-3 rounded-2xl border border-rose-100 bg-[#fff8f5] p-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#6f1028] text-white"><Phone className="h-5 w-5" /></span><div className="min-w-0 flex-1"><strong className="block text-sm text-[#54162a]">تحتاج مساعدة قبل التأكيد؟</strong><small className="mt-1 block text-xs font-medium leading-5 text-slate-600">{contactCount ? `تتوفر ${contactCount} ${contactCount === 1 ? "جهة تواصل" : "جهات تواصل"} لخدمتك.` : "تواصل مع فريق لحظة عند نشر أرقام الدعم."}</small></div><button type="button" onClick={onOpen} className="shrink-0 rounded-xl border border-rose-200 bg-white px-3 py-2 text-xs font-black text-[#6f1028] transition hover:bg-rose-50">تواصل معنا</button></div>;
}
      ...
      {screen === "home" ? (
        <>
          ...
          <section className="app-shell pb-10">
            ...
            <div className="mt-7 flex flex-wrap items-center justify-center gap-2"><button type="button" onClick={() => setSupportOpen(true)} className="inline-flex items-center gap-2 rounded-xl border border-rose-200 bg-white px-3 py-2 text-xs font-black text-[#6f1028] transition hover:bg-rose-50"><Phone className="h-4 w-4 text-[#ed665e]" /> تواصل معنا</button><button type="button" onClick={() => setAboutOpen(true)} className="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-bold text-slate-500 transition hover:bg-slate-50 hover:text-[#6f1028]"><CircleHelp className="h-4 w-4" /> حول التطبيق</button></div><div className="mt-5 flex items-center justify-center gap-2 text-xs text-slate-400"><Bike className="h-4 w-4 text-red-600" /><span>خدمة محلية مخصصة لمنبج</span></div>
          </section>
        </>
      ) : null}
