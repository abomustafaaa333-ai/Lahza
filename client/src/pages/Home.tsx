...
function ServiceIntroCarousel({ onActiveChange, onExplore }: { onActiveChange: (index: number) => void; onExplore: () => void }) {
  ...
  return <section className="service-intro-panel" aria-label="خدمات لحظة"><div className="service-intro-copy"><span className="service-intro-kicker">خدمات لحظة</span><strong>{slide.title}</strong><small>{slide.detail}</small><button type="button" className="service-intro-cta" onClick={onExplore}>استكشف الآن <ChevronLeft className="h-4 w-4" /></button></div><span className="service-intro-icon"><Icon className="h-7 w-7" /></span><div className="service-intro-dots">{slides.map((item, index) => <button key={item.title} type="button" className={index === active ? "service-intro-dot-active" : ""} onClick={() => changeActive(index)} aria-label={`الشريحة ${index + 1}`} />)}</div></section>;
}
