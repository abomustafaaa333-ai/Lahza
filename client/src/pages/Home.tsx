...
function SupportContactsDialog({ open, onOpenChange, contacts }: { open: boolean; onOpenChange: (open: boolean) => void; contacts: SupportContact[] }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent dir="rtl" className="w-[calc(100%-1.5rem)] max-w-md rounded-3xl border-0 bg-white p-5 shadow-2xl"><DialogHeader><DialogTitle className="flex items-center gap-2 text-right text-xl text-[#4a2618]"><Phone className="h-5 w-5 text-[#ff8438]" /> تواصل معنا</DialogTitle><DialogDescription className="text-right">اختر الطريقة المناسبة، وسيصل اتصالك مباشرة إلى فريق لحظة.</DialogDescription></DialogHeader><div className="mt-3 space-y-3">{contacts.length ? contacts.map(contact => <article key={contact.id} className="rounded-2xl border border-rose-100 bg-[#fffaf8] p-4"><strong className="block text-sm text-[#4a2618]">{contact.label}</strong><span dir="ltr" className="mt-1 block text-xs font-bold text-slate-500">{contact.phone}</span><div className="mt-3 flex flex-wrap gap-2">{contact.callEnabled ? <a href={`tel:${contact.phone}`} className="inline-flex items-center gap-2 rounded-xl bg-[#63301b] px-3 py-2 text-xs font-black text-white transition hover:bg-[#4a2618]"><Phone className="h-4 w-4" /> اتصال</a> : null}{contact.whatsappEnabled ? <a href={supportWhatsAppUrl(contact.phone)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white transition hover:bg-emerald-700"><MessageCircle className="h-4 w-4" /> واتساب</a> : null}</div></article>) : <div className="rounded-2xl border border-dashed border-rose-200 bg-rose-50 p-4 text-center text-sm font-bold leading-7 text-[#63301b]">لا توجد أرقام تواصل منشورة حالياً. سيضيفها مدير لحظة من لوحة التحكم قريباً.</div>}</div></DialogContent></Dialog>;
}
...
function PartnerOfferGallery({ slides, onOpen }: { slides: PartnerGallerySlide[]; onOpen: () => void }) {
  ...
  return <div className="hero-image-frame" aria-label="عروض مصورة من متاجر لحظة">{activeSlide ? <><button key={activeSlide.id} type="button" onClick={onOpen} className="hero-gallery-slide block h-full w-full cursor-pointer text-right" aria-label={`فتح العروض المميزة: ${activeSlide.name}`}><img src={activeSlide.imageUrl} alt={`عرض ${activeSlide.name} من ${activeSlide.partnerName}`} onError={event => { event.currentTarget.src = fallback; }} className="hero-offer-image" />{activeSlide.discountPercent ? <span className="hero-offer-discount">خصم {activeSlide.discountPercent}%</span> : null}<div className="hero-offer-caption"><strong>{activeSlide.name}</strong><span className="hero-offer-meta"><span>{activeSlide.partnerName}</span>{activeSlide.originalProductPrice || activeSlide.offerPrice || activeSlide.unitPrice ? <span className="hero-offer-prices"><del>{formatSyp(activeSlide.originalProductPrice || activeSlide.unitPrice)}</del><b>فقط {formatSyp(activeSlide.offerPrice || activeSlide.unitPrice)}</b></span> : null}</span></div></button>{slides.length > 1 ? <div className="hero-gallery-dots" aria-label="صور العروض">{slides.map((slide, index) => <button key={slide.id} type="button" onClick={() => setActiveIndex(index)} aria-label={`عرض الصورة ${index + 1}`} aria-current={index === activeIndex} className={index === activeIndex ? "hero-gallery-dot-active" : ""} />)}</div> : null}</> : <div className="hero-gallery-empty"><BadgePercent className="h-9 w-9" /><strong>عروض لحظة</strong><span>ترقّبوا أحدث العروض من متاجركم المفضلة</span><button type="button" onClick={onOpen} className="hero-gallery-cta">اكتشف العروض</button></div>}</div>;
}
function EntryGateDialog({ open, onChoose }: { open: boolean; onChoose: (role: "owner" | "partner" | "guest") => void }) {
  return <Dialog open={open} onOpenChange={() => undefined}><DialogContent showCloseButton={false} dir="rtl" className="w-[calc(100%-1.5rem)] max-w-md overflow-hidden rounded-[2rem] border-0 bg-gradient-to-b from-[#ffffff] to-white p-0 shadow-2xl"><div className="entry-gate-hero"><div className="entry-gate-brand"><img src="/assets/lahza-logo-option-4-header.png?v=8" alt="لحظة" /><span>كل شيء في لحظة</span></div><p>مرحباً بك في خدمات لحظة</p><small>اختر طريقة الدخول للمتابعة</small></div><div className="grid gap-3 p-5"><button type="button" onClick={() => onChoose("owner")} className="entry-gate-option"><span className="entry-gate-option-icon"><LayoutDashboard className="h-5 w-5" /></span><span><strong>دخول المالك</strong><small>إدارة التطبيق والطلبات والمتاجر</small></span><ChevronLeft className="h-5 w-5" /></button><button type="button" onClick={() => onChoose("partner")} className="entry-gate-option"><span className="entry-gate-option-icon"><Store className="h-5 w-5" /></span><span><strong>دخول الشريك</strong><small>إدارة متجرك ومنتجاتك وعروضك</small></span><ChevronLeft className="h-5 w-5" /></button><button type="button" onClick={() => onChoose("guest")} className="entry-gate-option entry-gate-option-guest"><span className="entry-gate-option-icon"><UserRound className="h-5 w-5" /></span><span><strong>الدخول كزائر</strong><small>تصفح المتاجر والعروض واطلب بسهولة</small></span><ChevronLeft className="h-5 w-5" /></button></div></DialogContent></Dialog>;
}

function InlineVerificationStatus({ status }: { status: "pending" | "approved" | "rejected" | null }) {
  if (!status) return null;
  const content = status === "pending" ? <><Loader2 className="h-5 w-5 animate-spin" /><span>جارٍ تأكيد الطلب</span></> : status === "approved" ? <><CheckCircle2 className="h-5 w-5" /><span>تم تسجيل حسابك بنجاح لدى لحظة وتم تأكيد طلبك</span></> : <><X className="h-5 w-5" /><span>تم رفض طلبك حاول مرة أخرى</span></>;
  return <div role="status" aria-live="polite" className={`mt-3 flex items-center justify-center gap-2 rounded-2xl px-4 py-3 text-center text-sm font-black leading-7 ${status === "pending" ? "border border-orange-200 bg-orange-50 text-[#8a431f]" : status === "approved" ? "border border-emerald-200 bg-emerald-50 text-emerald-800" : "border border-red-200 bg-red-50 text-red-800"}`}>{content}</div>;
}
...
export default function Home() {
  ...
  const featuredStoreCards = useMemo(() => {
    ...
    const uniqueStores = new Map<number, { name: string; category: LahzaCategory; note: string }>();
    for (const offer of partnerOffers) {
      const storeId = Number(offer.storeId ?? 0);
      const category = offer.storeCategory as LahzaCategory;
      if (storeId > 0 && !uniqueStores.has(storeId) && categoryImageByKey[category]) uniqueStores.set(storeId, { name: offer.storeName || "متجر مميز", category, note: "لديه عروض مميزة نشطة" });
    }
  ...
  const submitCheckout = async (skipAccountCheck = false) => {
    ...
    if (!isStaticDemo && !skipAccountCheck) {
      try {
        ...
        if (account.status === "rejected") {
          ...
          toast.error("تم رفض طلبك حاول مرة أخرى");
          return;
        }
  ...
  useEffect(() => {
    if (!pendingCustomerOpen) return;
    const status = accountStatusQuery.data?.status;
    if (status === "approved") {
      setPendingCustomerOpen(false);
      setVerificationResult("approved");
      toast.success("تم تسجيل حسابك بنجاح لدى لحظة وتم تأكيد طلبك");
      void submitCheckout(true);
    } else if (status === "rejected") {
      setPendingCustomerOpen(false);
      setVerificationResult("rejected");
    }
  }, [pendingCustomerOpen, accountStatusQuery.data?.status]);
  ...
  return (
    <main dir="rtl" className="lahza-app-shell min-h-screen text-slate-950" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
      ...
      {screen === "home" ? (
        <>
          <section className="app-shell pb-10">
            ...
            <section className="home-showcase-block"><div className="home-section-heading"><div><p className="section-eyebrow">اختيارات لحظة</p><h2 className="section-title">المتاجر المميزة</h2></div><button type="button" onClick={() => setScreen("delivery")}>عرض الكل <ChevronLeft className="h-4 w-4" /></button></div><div className="home-featured-row">{featuredStoreCards.length ? featuredStoreCards.map(store => <button key={store.name} type="button" className="home-featured-store" onClick={() => { setActiveCategory(store.category); setActiveCustomCategory(null); setSelectedStore(null); setScreen("stores"); }}><img src={categoryImageByKey[store.category]} alt="" loading="lazy" /><span className="home-store-icon"><LahzaCategoryIcon category={store.category} className="h-4 w-4" /></span><strong>{store.name}</strong><small>{store.note}</small><span className="home-store-open">لديه عروض</span></button>) : <div className="home-featured-empty">لا توجد متاجر لديها عروض مميزة حالياً</div>}</div></section>
            <section className="home-showcase-block"><div className="home-section-heading"><div><p className="section-eyebrow">مختارة من الكتالوج</p><h2 className="section-title">الأكثر طلباً</h2></div><button type="button" onClick={() => setScreen("delivery")}>عرض الكل <ChevronLeft className="h-4 w-4" /></button></div><div className="home-popular-grid">{homePopularProducts.map(product => <button key={product.name} type="button" className="home-popular-product" onClick={() => { setActiveCategory(product.category); setActiveCustomCategory(null); setSelectedStore(null); setScreen("stores"); }}><img src={categoryImageByKey[product.category]} alt="" loading="lazy" /><span className="home-product-favorite"><Sparkles className="h-3.5 w-3.5" /></span><span className="home-product-plus"><Plus className="h-4 w-4" /></span><strong>{product.name}</strong><small>{product.note}</small></button>)}</div></section>
      ...
      {screen === "storeOffers" && selectedStore ? <StoreOffersScreen store={selectedStore} offers={(isStaticDemo ? partnerOffers : (storeOffersQuery.data ?? []).filter(offer => offer.featuredStatus === "approved")) as CustomerOffer[]} loading={storeOffersQuery.isLoading && !isStaticDemo} onBack={() => setScreen("store")} onChoose={chooseOffer} /> : null}
      ...
      {screen === "checkout" ? (
        <>
          ...
          {rejectedAccount && !pendingCustomerOpen ? <section className="app-shell pb-4"><div className="rounded-3xl border border-red-200 bg-red-50 p-5 text-center shadow-sm"><X className="mx-auto h-10 w-10 text-red-600" /><h2 className="mt-3 text-lg font-black text-[#63301b]">تم رفض حسابك</h2><p className="mt-2 text-sm leading-7 text-red-800">تم رفض حسابك. حاول مرة أخرى بعد التواصل مع فريق لحظة عبر واتساب لإعادة التحقق من رقمك.</p><a href={lahzaSupportWhatsAppUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center justify-center gap-2 rounded-xl bg-[#25D366] px-4 py-3 text-sm font-black text-white"><MessageCircle className="h-5 w-5" /> واتساب فريق لحظة: <span dir="ltr">+963997311078</span></a></div></section> : null}
          ...
          {screen === "checkout" ? <div className="app-shell"><InlineVerificationStatus status={pendingCustomerOpen ? "pending" : verificationResult} /></div> : null}
        </>
      ) : null}
...
function StoreShareCard({ store }: { store: StoreOption }) {
  ...
  return <><section className="app-shell pb-2"><div className="rounded-3xl border border-rose-100 bg-gradient-to-l from-rose-50 to-white p-4 shadow-sm"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#63301b] text-white"><Share2 className="h-5 w-5" /></span><div className="min-w-0 flex-1"><strong className="block text-sm text-[#4a2618]">رابط الدخول إلى المتجر</strong><small className="mt-1 block text-xs leading-5 text-slate-600">شارك المتجر مع من تحب أو اعرض رمز QR لفتحه مباشرة.</small></div></div><div className="mt-4 grid grid-cols-2 gap-2"><Button type="button" onClick={() => void shareStore()} className="rounded-2xl bg-[#63301b] hover:bg-[#4a2618]"><Share2 className="h-4 w-4" /> مشاركة الرابط</Button><Button type="button" variant="outline" onClick={() => setQrOpen(true)} className="rounded-2xl border-rose-200 bg-white text-[#4a2618] hover:bg-rose-50"><QrCode className="h-4 w-4" /> رمز QR</Button></div>{!nativeApp ? <a href={appDownloadUrl} className="mt-2 flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-2.5 text-sm font-black text-red-700 transition hover:bg-red-100 active:scale-[0.98]" aria-label="تحميل تطبيق لحظة من صفحة المتجر"><Smartphone className="h-4 w-4" /> تحميل تطبيق لحظة</a> : null}</div></section><Dialog open={qrOpen} onOpenChange={setQrOpen}><DialogContent dir="rtl" className="w-[calc(100%-2rem)] max-w-sm rounded-3xl bg-white text-center"><DialogHeader><DialogTitle className="text-center text-xl text-[#4a2618]">رمز متجر {store.name}</DialogTitle><DialogDescription className="text-center">امسح الرمز لفتح صفحة المتجر والمنتجات مباشرة.</DialogDescription></DialogHeader><div className="mx-auto mt-3 rounded-3xl border border-slate-100 bg-white p-4 shadow-sm"><QRCodeSVG value={storeUrl} size={220} level="M" includeMargin /></div><p dir="ltr" className="mt-3 break-all text-center text-[0.68rem] text-slate-400">{storeUrl}</p><Button type="button" onClick={() => void shareStore()} className="mt-3 w-full rounded-2xl bg-red-600 hover:bg-red-700"><Share2 className="h-4 w-4" /> مشاركة رابط المتجر</Button></DialogContent></Dialog></>;
}
...
function StoreOffersScreen({ store, offers, loading, onBack, onChoose }: { store: StoreOption; offers: CustomerOffer[]; loading: boolean; onBack: () => void; onChoose: (offer: CustomerOffer) => void }) {
  return <><PageHeading eyebrow="عروض المتجر" title={`عروض ${store.name}`} detail="اضغط على العرض المختار ثم حدّد كميته فقط." onBack={onBack} /><section className="app-shell pb-12">{loading ? <div className="rounded-3xl bg-slate-50 p-6 text-center text-sm text-slate-500">جارٍ تحميل العروض...</div> : offers.length ? <div className="grid gap-4 sm:grid-cols-2">{offers.map(offer => <button key={offer.id} onClick={() => onChoose(offer)} className="overflow-hidden rounded-3xl border border-amber-200 bg-white text-right shadow-sm transition hover:-translate-y-0.5 active:scale-[.98]"><CustomerOfferArtwork offer={offer} compact /><span className="block p-4"><span className="inline-flex rounded-full bg-red-100 px-3 py-1 text-xs font-black text-red-800">عرض مميز</span><small className="mt-3 block text-red-600">اضغط لاختيار الكمية</small></span></button>)}</div> : <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center"><BadgePercent className="mx-auto h-7 w-7 text-red-600" /><h2 className="mt-3 text-lg font-black text-[#4a2618]">لا توجد عروض نشطة لهذا المتجر</h2></div>}</section></>;
}
...
function CustomerOfferArtwork({ offer, compact = false }: { offer: CustomerOffer; compact?: boolean }) {
  const productImage = offer.productImageUrl || offer.imageUrl || "";
