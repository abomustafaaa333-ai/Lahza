...
import { ArrowLeft, BadgePercent, BellRing, Bike, CakeSlice, CarFront, CheckCircle2, ChevronLeft, CircleHelp, ClipboardList, Clock3, CreditCard, Download, Fuel, HandCoins, LayoutDashboard, Loader2, LocateFixed, LogOut, MapPinCheck, MapPinned, MessageCircle, Minus, PackageCheck, PackagePlus, Pencil, Phone, Pill, Plus, QrCode, Search, Share2, Shirt, ShoppingBasket, Smartphone, Sparkles, Store, Trash2, Truck, UserRound, UtensilsCrossed, Wheat, X } from "lucide-react";
...
type Screen = "home" | "delivery" | "stores" | "store" | "productQuantity" | "storeOffers" | "offerQuantity" | "taxi" | "intercity" | "offers" | "checkout" | "wosselLi" | "driverSearch" | "orderTracking" | "account" | "customerOrders";
type PromotionPreview = { code: string; kind: "discount" | "referral"; percent: number; discountAmount: number; itemsTotal: number };
type SubmittedOrder = { id: number; customerPhone: string; orderType: "delivery" | "taxi" | "wossel_li"; customerName: string; status: "pending" | "confirmed" | "preparing" | "on_the_way" | "completed" | "cancelled" | "rejected"; totalAmount: number; deliveryFee: number; deliveryAddress: string; paymentMethod: "cash" | "sham_cash"; eta: string; lines: CartLine[]; notes: string; customerLat?: number | null; customerLng?: number | null; driverLocation?: { name: string; latitude: number; longitude: number; updatedAt: string | Date } | null };
...
function ServiceIntroCarousel({ onActiveChange, onExplore, onGateway, city }: { onActiveChange: (index: number) => void; onExplore: () => void; onGateway: () => void; city: CityKey }) {
  ...
  return <section className="service-intro-panel" aria-label="خدمات لحظة"><div className="service-intro-copy"><span className="service-intro-kicker">خدمات لحظة</span><span className="service-intro-tagline">{CITY_LABELS[city]} بين يديك</span><strong>{slide.title}</strong><small>{slide.detail}</small><button type="button" className="service-intro-cta" onClick={onExplore}>استكشف الآن <ChevronLeft className="h-4 w-4" /></button></div><div className="service-intro-offer-mark" aria-label="أقوى العروض لدى لحظة"><span className="service-intro-offer-mark-icon"><BadgePercent className="h-5 w-5" /></span><strong>أقوى العروض</strong><b>لدى لحظة</b>{city === "jarabulus" ? <button type="button" className="service-gateway-button" onClick={onGateway}>اطلب من منبج <ChevronLeft className="h-4 w-4" /></button> : null}</div><div className="service-intro-dots">{slides.map((item, index) => <button key={item.title} type="button" className={index === active ? "service-intro-dot-active" : ""} onClick={() => changeActive(index)} aria-label={`الشريحة ${index + 1}`} />)}</div></section>;
}
...
export default function Home() {
  ...
  const driverTrackingToken = useMemo(() => new URLSearchParams(window.location.search).get("driverTrack") || "", []);
  const driverTrackingActiveQuery = trpc.lahza.driverTracking.active.useQuery({ token: driverTrackingToken }, { enabled: !isStaticDemo && driverTrackingToken.length >= 32, retry: false, refetchInterval: 10_000 });
  const updateDriverLocation = trpc.lahza.driverTracking.update.useMutation();
  useEffect(() => {
    if (!driverTrackingToken || !driverTrackingActiveQuery.data?.active || !driverTrackingActiveQuery.data.orderId || !navigator.geolocation) return;
    const watchId = navigator.geolocation.watchPosition(position => {
      updateDriverLocation.mutate({ token: driverTrackingToken, orderId: driverTrackingActiveQuery.data.orderId!, latitude: position.coords.latitude, longitude: position.coords.longitude });
    }, error => console.warn("Driver location permission/update failed", error), { enableHighAccuracy: true, maximumAge: 10_000, timeout: 20_000 });
    return () => navigator.geolocation.clearWatch(watchId);
  }, [driverTrackingToken, driverTrackingActiveQuery.data?.active, driverTrackingActiveQuery.data?.orderId]);
  ...
  const submitCheckout = async () => {
    ...
    if (isWosselLi && (!pickup.trim() || !/^\+9639\d{8}$/.test(wosselPickupContactPhone) || !wosselItemDescription.trim() || !deliveryAddress.trim())) {
      toast.error("أكمل عنوان ورقم الاستلام ووصف الغرض وعنوان التسليم");
      return;
    }
    ...
    createOrder.mutate({
      ...
      pickupLocation: isTaxi || isWosselLi ? pickup : undefined,
      ...
      itemWeight: isWosselLi ? wosselItemWeight.trim() : undefined,
  ...
  if (driverTrackingToken) return <main dir="rtl" className="customer-auth-loading"><img src="/assets/lahza-logo.svg" alt="لحظة" /><strong>تفعيل تتبع المندوب</strong><span>{driverTrackingActiveQuery.data?.active ? "تم تفعيل مشاركة الموقع للطلب النشط." : "ستبدأ مشاركة موقعك تلقائياً عند قبول طلب نشط."}</span><small>يمكنك إبقاء هذه الصفحة مفتوحة أثناء تنفيذ الطلب.</small></main>;
  ...
  return (
    <main dir="rtl" className="lahza-app-shell min-h-screen text-slate-950" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
      ...
      {screen === "home" ? <Header onSearch={() => setSearchOpen(true)} onExplore={openFeaturedOffers} onGateway={() => { setGatewayMode(true); setScreen("stores"); }} searchPlaceholder={searchPlaceholder} city={selectedCity ?? "manbij"} /> : null}
      ...
      {screen === "home" ? (
        <>
          <button type="button" className="wossel-li-callout" role="status" onClick={() => { setCheckoutMode("wossel_li"); setScreen("checkout"); }}><span>هل تريد نقل غرض من مكان إلى مكان؟</span><strong>افتح خدمة وصّل لي</strong></button>
...
function CustomerAccountScreen({ session, orders, onBack, onLogout, onPhoneSubmit, savingPhone, onOpenOrders, onOpenOrder }: { session: CustomerAuthSession; orders: CustomerHistoryOrder[]; onBack: () => void; onLogout: () => void; onPhoneSubmit: (phone: string) => void; savingPhone: boolean; onOpenOrders: () => void; onOpenOrder?: () => void }) {
...
function DriverDashboard({ session, onLogout }: { session: DriverDashboardSession; onLogout: () => void }) {
  ...
  const updateLocation = trpc.lahza.driverTracking.update.useMutation();
  const activeOrder = (driverSessionQuery.data?.assignments ?? session.assignments).find(order => ["accepted", "picked_up"].includes(order.assignmentStatus));
  useEffect(() => {
    if (!session.trackingToken || !activeOrder || !navigator.geolocation) return;
    const watchId = navigator.geolocation.watchPosition(position => updateLocation.mutate({ token: session.trackingToken!, orderId: activeOrder.orderId, latitude: position.coords.latitude, longitude: position.coords.longitude }), error => console.warn("Driver GPS error", error), { enableHighAccuracy: true, maximumAge: 10_000, timeout: 20_000 });
    return () => navigator.geolocation.clearWatch(watchId);
  }, [session.trackingToken, activeOrder?.orderId, activeOrder?.assignmentStatus]);
  const assignments = driverSessionQuery.data?.assignments ?? session.assignments;
  return <main dir="rtl" className="min-h-screen bg-[#fffaf6] px-4 py-8 text-[#4a2618]"><section className="mx-auto max-w-lg space-y-5"><div className="rounded-[2rem] bg-[#63301b] p-6 text-white shadow-xl"><p className="text-sm font-bold text-orange-100">لحظة · بوابة المندوب</p><h1 className="mt-2 text-2xl font-black">مرحباً {session.name}</h1><p className="mt-2 text-sm text-orange-100" dir="ltr">{session.phone}</p><div className="mt-5 flex items-center justify-between rounded-2xl bg-white/10 p-3 text-sm font-bold"><span>حالة التتبع</span><span className={activeOrder ? "text-emerald-200" : "text-orange-100"}>{activeOrder ? "نشط تلقائياً" : "بانتظار طلب مقبول"}</span></div></div><section className="rounded-3xl border border-orange-100 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><p className="text-xs font-bold text-slate-400">الطلبات الحالية</p><h2 className="mt-1 text-xl font-black">طلباتك المسندة</h2></div><Bike className="h-7 w-7 text-[#e95e2a]" /></div>{assignments.length ? <div className="mt-4 space-y-3">{assignments.map(order => <article key={order.orderId} className="rounded-2xl bg-slate-50 p-4"><div className="flex items-center justify-between gap-3"><strong>طلب #{order.orderId}</strong><span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-teal-700">{order.assignmentStatus === "accepted" ? "مقبول" : order.assignmentStatus === "picked_up" ? "تم الاستلام" : "مسند"}</span></div><p className="mt-2 text-sm font-bold text-slate-600">العميل: {order.customerName}</p><p className="mt-1 text-xs leading-6 text-slate-500">{order.locationText || "موقع التسليم محدد عبر GPS"}</p></article>)}</div> : <p className="mt-5 rounded-2xl bg-slate-50 p-5 text-center text-sm font-bold leading-7 text-slate-500">لا توجد طلبات نشطة حالياً. سيبدأ التتبع تلقائياً بعد قبول طلب عبر واتساب.</p>}</section><button type="button" onClick={onLogout} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-rose-200 bg-white px-5 py-3 text-sm font-black text-[#63301b]"><LogOut className="h-4 w-4" /> تسجيل الخروج</button></section></main>;
}
...
function OrderTrackingScreen({ order, loading, onHome, onOpenCart }: { order: SubmittedOrder; loading: boolean; onHome: () => void; onOpenCart: () => void }) {
  ...
  const remainingMeters = order.driverLocation && order.customerLat !== null && order.customerLng !== null && order.customerLat !== undefined && order.customerLng !== undefined ? straightLineDistanceMeters(order.driverLocation.latitude, order.driverLocation.longitude, order.customerLat, order.customerLng) : null;
  const remainingKm = remainingMeters === null ? null : Math.max(0.1, remainingMeters / 1000);
  const remainingEta = remainingKm === null ? null : Math.max(1, Math.ceil(remainingKm * 3));
  return <><PageHeading eyebrow={`متابعة الطلب #${order.id}`} title={statusTitle} detail={loading ? "جارٍ تحديث حالة طلبك..." : "سنحدّث هذه الصفحة تلقائياً عند انتقال طلبك إلى مرحلة جديدة."} onBack={onHome} /><section className="app-shell space-y-5 pb-10"><div className="tracking-hero"><span><Clock3 className="h-6 w-6" /></span><div><strong>{order.status === "on_the_way" ? "المندوب في الطريق" : "وقت الوصول التقديري"}</strong><b>{remainingEta !== null ? `${remainingEta} دقيقة تقريباً` : order.eta}</b><small>{order.orderType === "delivery" ? `التوصيل إلى: ${order.deliveryAddress}` : order.deliveryAddress}</small></div><span className="tracking-live">{loading ? "تحديث" : "مباشر"}</span></div>{order.driverLocation && remainingKm !== null ? <div className="tracking-card border-emerald-200 bg-emerald-50/60"><div className="tracking-card-title"><MapPinned className="h-5 w-5 text-emerald-700" /><span>تتبع المندوب</span><small className="mr-auto text-xs text-emerald-700">آخر تحديث {new Date(order.driverLocation.updatedAt).toLocaleTimeString("ar-SY", { hour: "2-digit", minute: "2-digit" })}</small></div><p className="text-sm font-bold leading-7 text-emerald-950">المندوب {order.driverLocation.name} يبعد عنك تقريباً <strong>{remainingKm.toFixed(1)} كم</strong>، والوقت المتوقع للوصول <strong>{remainingEta} دقائق</strong>.</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-emerald-100"><div className="h-full rounded-full bg-emerald-600 transition-all" style={{ width: `${Math.max(8, Math.min(100, 100 - remainingKm * 10))}%` }} /></div></div> : <div className="tracking-card"><div className="tracking-card-title"><MapPinned className="h-5 w-5" /><span>تتبع الطلب</span></div><p className="text-sm font-bold leading-7 text-slate-500">سيظهر موقع المندوب والوقت المتبقي فور موافقته على الطلب وبدء مشاركة موقعه.</p></div>}<div className="tracking-card"><div className="tracking-card-title"><PackageCheck className="h-5 w-5" /><span>حالة طلبك</span></div><div className="tracking-steps">{stages.map((stage, index) => { const Icon = stage.icon; const active = index <= currentIndex; return <div key={stage.key} className={active ? "tracking-step tracking-step-active" : "tracking-step"}><span><Icon className="h-4 w-4" /></span><div><strong>{stage.title}</strong><small>{stage.detail}</small></div></div>; })}</div></div><div className="tracking-card"><div className="tracking-card-title"><ClipboardList className="h-5 w-5" /><span>تقرير الطلب</span></div><div className="tracking-lines">{order.lines.length ? order.lines.map(line => <div key={line.id}><span>{line.itemName} <small>× {line.quantity} {line.unit}</small></span><strong>{line.priceKnown ? formatSyp(lineTotal(line)) : "يؤكد لاحقاً"}</strong></div>) : <p>طلب سيارة أجرة — يحدد السائق تفاصيل الرحلة عند التأكيد.</p>}</div><div className="tracking-total"><div><span>رسوم التوصيل</span><strong>{order.deliveryFee > 0 ? formatSyp(order.deliveryFee) : "تحدد بعد إسناد المندوب"}</strong></div><div><span>إجمالي الطلب</span><strong>{formatSyp(order.totalAmount)}</strong></div></div></div><div className="tracking-info-grid"><article><LocateFixed className="h-5 w-5" /><span>عنوان التوصيل</span><strong>{order.deliveryAddress}</strong></article><article><CreditCard className="h-5 w-5" /><span>الدفع</span><strong>{order.paymentMethod === "sham_cash" ? "شام كاش" : "نقداً عند الاستلام"}</strong></article></div>{order.notes ? <div className="tracking-note"><CircleHelp className="h-4 w-4" />{order.notes}</div> : null}<div className="grid grid-cols-2 gap-3"><button type="button" onClick={onHome} className="tracking-secondary-button">الرئيسية</button><button type="button" onClick={onOpenCart} className="primary-full-button">طلب جديد <Plus className="h-4 w-4" /></button></div></section></>;
}
