        ...
        <>
          ...
          <section className="app-shell pb-10 home-discover-section">
            ...
            <div className="home-category-row" dir="rtl" aria-label="أقسام لحظة">
              {homeDiscoverCategories.map(item => <button key={item.key} type="button" className="home-category-card" onClick={() => { if (!item.category) { setScreen("delivery"); return; } setSelectedStore(null); setSelectedProduct(null); setActiveCustomCategory(null); setActiveCategory(item.category); setScreen("stores"); }}><span className="home-category-emoji" aria-hidden="true">{item.icon}</span><span>{item.title}</span></button>)}
