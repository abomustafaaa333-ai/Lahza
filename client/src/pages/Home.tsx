        ...
        <>
          <section className="app-shell pb-10">
            ...
            <div className="home-category-row" aria-label="أقسام لحظة">
              <button type="button" className="home-category-card home-category-all" onClick={() => setScreen("delivery")}><span className="home-category-mini-grid" aria-hidden="true"><span><img src={categoryImageByKey.restaurants} alt="" /></span><span><img src={categoryImageByKey.groceries} alt="" /></span><span><img src={categoryImageByKey.pharmacy} alt="" /></span><span><img src={categoryImageByKey.bakery} alt="" /></span><b className="home-category-all-label">الكل</b></span></button>
            ...
            <section className="home-showcase-block"><div className="home-section-heading"><div><p className="section-eyebrow">اختيارات لحظة</p><h2 className="section-title">المتاجر المميزة</h2></div><button type="button" onClick={() => setScreen("delivery")}>عرض الكل <ChevronLeft className="h-4 w-4" /></button></div><div className="home-featured-row">{featuredStoreCards.length ? featuredStoreCards.map(store => <button key={store.name} type="button" className="home-featured-store" onClick={() => { setActiveCategory(store.category); setActiveCustomCategory(null); setSelectedStore(null); setScreen("stores"); }}><img src={categoryImageByKey[store.category]} alt="" loading="lazy" /><span className="home-store-icon"><LahzaCategoryIcon category={store.category} className="h-4 w-4" /></span><strong>{store.name}</strong><span className="home-store-rating" aria-label="تقييم 5 من 5">★★★★★</span><small>{store.note}</small><span className="home-store-open">لديه عروض</span></button>) : <div className="home-featured-empty">لا توجد متاجر لديها عروض مميزة حالياً</div>}</div></section>
