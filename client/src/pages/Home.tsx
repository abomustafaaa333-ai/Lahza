...
export default function Home() {
  ...
  return (
    <main dir="rtl" className="lahza-app-shell min-h-screen text-slate-950" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
      ...
      {screen === "offers" ? (
        <OfferDestinationScreen offers={partnerOffers as CustomerOffer[]} gallerySlides={partnerGallerySlides} onBack={goHome} loading={partnerOffersQuery.isLoading && !isStaticDemo} focusedOfferId={focusedOfferId} onChoose={chooseOffer} />
      ) : null}
...
function OfferDestinationScreen({ offers, gallerySlides, loading, onBack, focusedOfferId, onChoose }: { offers: CustomerOffer[]; gallerySlides: PartnerGallerySlide[]; loading: boolean; onBack: () => void; focusedOfferId: number | null; onChoose: (offer: CustomerOffer) => void }) {
  ...
  return <><PageHeading eyebrow="عروض لحظة" title="العروض المميزة" detail="هذه العروض اعتمدها فريق لحظة. اختر العرض ثم حدّد الكمية وأضفه إلى السلة." onBack={onBack} /><div className="global-offer-bar offers-page-gallery"><PartnerOfferGallery slides={gallerySlides} onOpen={() => undefined} /></div><section className="app-shell pb-12">{loading ? <div className="rounded-3xl bg-slate-50 p-6 text-center text-sm text-slate-500">جارٍ تحميل العروض المميزة...</div> : offers.length ? <div className="grid gap-4 sm:grid-cols-2">{offers.map(offerCard)}</div> : <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center"><BadgePercent className="mx-auto h-7 w-7 text-red-600" /><h2 className="mt-3 text-lg font-black text-[#4a2618]">لا توجد عروض مميزة الآن</h2><p className="mt-2 text-sm leading-7 text-slate-500">سيظهر هنا كل عرض بعد اعتماده كعرض مميز من فريق لحظة.</p></div>}</section></>;
}
