...
function CustomerOfferArtwork({ offer, compact = false }: { offer: CustomerOffer; compact?: boolean }) {
  ...
  return <div className={`lahza-offer-artwork ${compact ? "lahza-offer-artwork-compact" : ""}`}><div className="lahza-offer-artwork-glow" /><span className="lahza-offer-artwork-brand">لحظة</span><span className="lahza-offer-artwork-badge">خصم {offer.discountPercent || "—"}%</span><div className="lahza-offer-artwork-copy"><small>{offer.storeName ?? offer.partnerName}</small><strong>{offer.text}</strong><span>{offer.productName ?? "عرض مميز"}</span><div className="lahza-offer-artwork-prices">{originalPrice > 0 ? <del>{formatSyp(originalPrice)}</del> : null}{discountedPrice > 0 ? <b>{formatSyp(discountedPrice)}</b> : null}</div></div><div className="lahza-offer-artwork-product">{productImage ? <img src={productImage} alt={offer.productName ?? offer.text} /> : <BadgePercent className="h-12 w-12" />}</div></div>;
}
