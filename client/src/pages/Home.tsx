...
function PartnerOfferGallery({ slides, onOpen }: { slides: PartnerGallerySlide[]; onOpen: () => void }) {
  ...
  return <div className="hero-image-frame" aria-label="عروض مصورة من متاجر لحظة">{slides.length ? <><div className="hero-gallery-track" style={{ transform: `translateX(-${activeIndex * 100}%)` }}>{slides.map(slide => <button key={slide.id} type="button" onClick={onOpen} className="hero-gallery-slide block w-full shrink-0 cursor-pointer text-right" aria-label={`فتح العروض المميزة: ${slide.name}`}><img src={slide.imageUrl} alt={`عرض ${slide.name} من ${slide.partnerName}`} onError={event => { event.currentTarget.style.display = "none"; }} className="hero-offer-image" />{slide.discountPercent ? <span className="hero-offer-discount">خصم {slide.discountPercent}%</span> : null}<div className="hero-offer-caption"><strong>{slide.name}</strong><span>{slide.partnerName}</span>{slide.originalProductPrice || slide.offerPrice || slide.unitPrice ? <span className="hero-offer-prices"><del>{formatSyp(slide.originalProductPrice || slide.unitPrice)}</del><b>فقط {formatSyp(slide.offerPrice || slide.unitPrice)}</b></span> : null}</div></button>)}</div>{slides.length > 1 ? <div className="hero-gallery-dots" aria-label="صور العروض">{slides.map((slide, index) => <button key={slide.id} type="button" onClick={() => setActiveIndex(index)} aria-label={`عرض الصورة ${index + 1}`} aria-current={index === activeIndex} className={index === activeIndex ? "hero-gallery-dot-active" : ""} />)}</div> : null}</> : <div className="hero-gallery-empty"><BadgePercent className="h-9 w-9" /><strong>عروض لحظة</strong><span>ترقّبوا أحدث العروض من متاجركم المفضلة</span><button type="button" onClick={onOpen} className="hero-gallery-cta">اكتشف العروض</button></div>}</div>;
}
