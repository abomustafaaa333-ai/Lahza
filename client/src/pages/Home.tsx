...
function PendingCustomerDialog({ open }: { open: boolean }) {
  return <Dialog open={open} onOpenChange={() => undefined}><DialogContent showCloseButton={false} dir="rtl" className="w-[calc(100%-1.5rem)] max-w-md rounded-3xl border-0 bg-[#fffaf7] p-6 text-center shadow-2xl"><DialogHeader><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-orange-100 text-[#f26d31]"><Loader2 className="h-8 w-8 animate-spin" /></div><DialogTitle className="mt-4 text-center text-xl text-[#4a2618]">جارٍ التحقق من حسابك</DialogTitle><DialogDescription className="mt-2 text-center leading-8">تم تسجيل طلبك، وسيبقى معلّقاً حتى يؤكد المالك أو أحد المشرفين حسابك.</DialogDescription></DialogHeader><div className="mt-5 rounded-2xl border border-orange-100 bg-white p-4 text-sm leading-8 text-[#63301b]"><strong className="block">لا حاجة لإعادة إرسال الطلب</strong><span className="mt-1 block text-slate-600">ستتحدث هذه الشاشة تلقائياً. عند تأكيد الحساب سيتابع طلبك مباشرة، وعند رفضه ستظهر لك رسالة الرفض.</span></div></DialogContent></Dialog>;
}
...
export default function Home() {
    ...
    if (!isStaticDemo && !skipAccountCheck) {
      try {
        ...
        if (account.status === "rejected") {
          toast.error("تم رفض حسابك، تواصل مع فريق لحظة عبر واتساب");
          return;
        }
  ...
  useEffect(() => {
    ...
    if (status === "approved") {
      ...
      toast.success("تم تأكيد حسابك، جارٍ متابعة طلبك الآن");
  ...
  return (
    <main dir="rtl" className="lahza-app-shell min-h-screen text-slate-950" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
      ...
      <PendingCustomerDialog open={pendingCustomerOpen} />
