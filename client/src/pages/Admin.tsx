...
function DeliverySettingsPanel() {
  ...
  return <section className="admin-section"><div className="admin-section-heading"><div><p>صلاحية المالك والمشرف</p><h2>نسب رسوم التوصيل</h2></div><MapPinned className="h-5 w-5 text-blue-900" /></div><p className="settings-copy">تحسب الرسوم من قيمة المنتجات فقط، ثم تضاف إلى الإجمالي النهائي. لا تدخل رسوم التوصيل في الحد الأدنى البالغ 300 ليرة سورية جديدة. وتحدد النسبة الثالثة خصم مكافأة العميل عند استبدال كل 10 نقاط.</p><div className="grid gap-3 sm:grid-cols-2"><div><Label>نسبة رسوم طلبات منبج (%)</Label><Input inputMode="numeric" value={manbijPercent} onChange={event => setManbijPercent(event.target.value.replace(/\D/g, ""))} placeholder="15" /></div><div><Label>نسبة رسوم طلبات جرابلس (%)</Label><Input inputMode="numeric" value={jarabulusPercent} onChange={event => setJarabulusPercent(event.target.value.replace(/\D/g, ""))} placeholder="30" /></div><div><Label>نسبة مكافأة كل 10 نقاط (%)</Label><Input inputMode="numeric" value={pointsRewardPercent} onChange={event => setPointsRewardPercent(event.target.value.replace(/\D/g, ""))} placeholder="5" /></div></div><Button disabled={update.isPending || !manbijPercent || !jarabulusPercent || pointsRewardPercent === ""} onClick={() => update.mutate({ manbijPercent: Number(manbijPercent), jarabulusPercent: Number(jarabulusPercent), pointsRewardPercent: Number(pointsRewardPercent) })} className="mt-4 rounded-xl bg-blue-900 hover:bg-blue-950"><MapPinned className="h-4 w-4" /> {update.isPending ? "جارٍ الحفظ..." : "حفظ نسب التوصيل"}</Button></section>;
}
