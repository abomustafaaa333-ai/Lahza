...
function PartnerStoreManagement() {
  ...
  return <div className="space-y-5"><section className="admin-section"><div className="admin-section-heading"><div><p>صلاحية المالك فقط</p><h2>إنشاء متجر وتعيين شريك</h2></div><Store className="h-5 w-5 text-red-600" /></div><p className="settings-copy">اختر الشريك الذي سيدخل إلى هذا المتجر فقط. يمكن ترك المتجر بلا شريك إلى أن تعيّن حساباً مناسباً.</p><div className="grid gap-3 md:grid-cols-4"><div><Label>اسم المتجر</Label><Input value={name} onChange={event => setName(event.target.value)} placeholder="مثال: حلويات الشام" /></div><div><Label>القسم</Label><select value={category} onChange={event => setCategory(event.target.value as LahzaCategory)} className="form-select">{(Object.keys(categoryMeta) as LahzaCategory[]).map(key => <option key={key} value={key}>{categoryMeta[key].title}</option>)}</select></div><div><Label>الشريك المعيّن</Label><select value={partnerId} onChange={event => setPartnerId(event.target.value)} className="form-select"><option value="">من دون شريك حالياً</option>{partners.map(partner => <option key={partner.id} value={partner.id}>{partner.name} · {partner.username}</option>)}</select></div><div><Label>ترتيب الظهور</Label><Input inputMode="numeric" value={sortOrder} onChange={event => setSortOrder(event.target.value.replace(/\D/g, ""))} /></div></div><Button disabled={create.isPending || !name.trim()} onClick={() => create.mutate({ name: name.trim(), category, partnerId: partnerId ? Number(partnerId) : null, active: true, sortOrder: Number(sortOrder || 0) })} className="mt-4 rounded-xl bg-red-600 hover:bg-red-700"><Store className="h-4 w-4" /> إضافة المتجر</Button></section><section className="admin-section"><div className="admin-section-heading"><div><p>صلاحيات المتاجر</p><h2>المتاجر والشركاء المعيّنون</h2></div><span className="admin-note">اختر متجراً لإدارة منتجاته</span></div>{stores.length ? <div className="grid gap-3 lg:grid-cols-2">{stores.map(store => <AssignedStoreRow key={store.id} store={store} partners={partners} selected={selectedStore?.id === store.id} saving={saving} onSelect={() => setSelectedStoreId(store.id)} onSave={values => update.mutate(values)} onRemove={() => { if (confirm(`حذف متجر ${store.name}؟`)) remove.mutate({ id: store.id }); }} />)}</div> : <Empty icon={Store} title="لا توجد متاجر بعد" text="أنشئ متجراً ثم عيّن شريكاً له ليتمكن من الدخول وإدارة منتجاته." />}</section>{selectedStore ? <section className="admin-section"><div className="admin-section-heading"><div><p>{categoryMeta[selectedStore.category].title}</p><h2>منتجات {selectedStore.name}</h2></div><Button variant="outline" size="sm" onClick={() => setSelectedStoreId(null)}>تغيير المتجر</Button></div><div className="grid gap-3 md:grid-cols-3"><div><Label>اسم المنتج</Label><Input value={productName} onChange={event => setProductName(event.target.value)} /></div><div><Label>السعر (ل.س جديدة)</Label><Input inputMode="numeric" value={productPrice} onChange={event => setProductPrice(event.target.value.replace(/\D/g, ""))} /></div><div className="flex items-end"><Button disabled={createProduct.isPending || !productName.trim() || productPrice === ""} onClick={() => createProduct.mutate({ name: productName.trim(), category: selectedStore.category, storeId: selectedStore.id, unit: "وحدة", price: Number(productPrice), available: true })} className="w-full rounded-xl bg-blue-900 hover:bg-blue-950"><PackagePlus className="h-4 w-4" /> إضافة المنتج</Button></div></div><div className="mt-5 space-y-3">{productList.length ? productList.map(product => <CatalogRow key={product.id} product={product} onSave={values => updateProduct.mutate({ ...values, storeId: selectedStore.id })} onRemove={() => removeProduct.mutate({ id: product.id })} saving={saving} />) : <Empty icon={PackageSearch} title="لا توجد منتجات في هذا المتجر" text="أضف منتجات للمتجر ليظهر محتواها للعميل وللشريك المعيّن." />}</div></section> : null}</div>;
}

function AssignedStoreRow({ store, partners, selected, saving, onSelect, onSave, onRemove }: { store: { id: number; name: string; category: LahzaCategory; partnerId: number | null; active: boolean; sortOrder: number }; partners: Array<{ id: number; name: string; username: string }>; selected: boolean; saving: boolean; onSelect: () => void; onSave: (values: { id: number; name: string; category: LahzaCategory; partnerId: number | null; active: boolean; sortOrder: number }) => void; onRemove: () => void }) {
  ...
  const values = (active = store.active) => ({ id: store.id, name: name.trim(), category, partnerId: partnerId ? Number(partnerId) : null, active, sortOrder: Number(sortOrder || 0) });
  return <article className={`rounded-2xl border p-4 ${selected ? "border-blue-300 bg-blue-50" : "border-slate-100 bg-white"}`}><div className="grid gap-2 md:grid-cols-2"><div><Label>اسم المتجر</Label><Input value={name} onChange={event => setName(event.target.value)} /></div><div><Label>القسم</Label><select value={category} onChange={event => setCategory(event.target.value as LahzaCategory)} className="form-select">{(Object.keys(categoryMeta) as LahzaCategory[]).map(key => <option key={key} value={key}>{categoryMeta[key].title}</option>)}</select></div><div><Label>الشريك المعيّن</Label><select value={partnerId} onChange={event => setPartnerId(event.target.value)} className="form-select"><option value="">من دون شريك</option>{partners.map(partner => <option key={partner.id} value={partner.id}>{partner.name} · {partner.username}</option>)}</select></div><div><Label>ترتيب الظهور</Label><Input inputMode="numeric" value={sortOrder} onChange={event => setSortOrder(event.target.value.replace(/\D/g, ""))} /></div></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={saving || !name.trim()} onClick={() => onSave(values())} className="rounded-lg bg-blue-50 px-3 py-2 text-xs font-bold text-blue-900">حفظ التعيين</button><button type="button" disabled={saving} onClick={onSelect} className="rounded-lg bg-blue-900 px-3 py-2 text-xs font-bold text-white">إدارة المنتجات</button><button type="button" disabled={saving} onClick={() => onSave(values(!store.active))} className={`rounded-lg px-3 py-2 text-xs font-bold ${store.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{store.active ? "ظاهر للعميل" : "مخفي عن العميل"}</button><button type="button" disabled={saving} onClick={onRemove} className="rounded-lg bg-red-50 px-3 py-2 text-red-600"><Trash2 className="h-4 w-4" /></button></div></article>;
}
...
function PartnersPanel() {
  ...
  const [username, setUsername] = useState("");
  const [password, setPassword] = useState("");
  const partnerLoginUrl = `${window.location.origin}/partner`;
  const copyPartnerLoginUrl = async () => {
    try {
      await navigator.clipboard.writeText(partnerLoginUrl);
      toast.success("تم نسخ رابط دخول الشريك");
    } catch {
      toast.error("تعذر نسخ الرابط؛ افتحه ثم انسخه من شريط العنوان");
    }
  };
