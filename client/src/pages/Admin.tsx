...
function StoreRow({ store, selected, saving, onSelect, onSave, onRemove }: { store: { id: number; name: string; category: LahzaCategory; active: boolean; sortOrder: number }; selected: boolean; saving: boolean; onSelect: () => void; onSave: (values: { id: number; name: string; category: LahzaCategory; active: boolean; sortOrder: number }) => void; onRemove: () => void }) {
  ...
  const [category, setCategory] = useState<LahzaCategory>(store.category);
  const [sortOrder, setSortOrder] = useState(String(store.sortOrder));
  return <div className={`rounded-2xl border p-4 ${selected ? "border-rose-300 bg-rose-50" : "border-slate-100 bg-white"}`}><div className="grid gap-2 sm:grid-cols-[1.2fr_1fr_86px_auto_auto]"><Input value={name} onChange={event => setName(event.target.value)} /><select value={category} onChange={event => setCategory(event.target.value as LahzaCategory)} className="form-select">{(Object.keys(categoryMeta) as LahzaCategory[]).map(key => <option key={key} value={key}>{categoryMeta[key].title}</option>)}</select><Input inputMode="numeric" value={sortOrder} onChange={event => setSortOrder(event.target.value.replace(/\D/g, ""))} /><button type="button" disabled={saving || !name.trim()} onClick={() => onSave({ id: store.id, name: name.trim(), category, active: store.active, sortOrder: Number(sortOrder || 0) })} className="rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-[#63301b]">حفظ</button><button type="button" disabled={saving} onClick={onRemove} className="rounded-lg bg-red-50 px-3 py-2 text-red-600"><Trash2 className="h-4 w-4" /></button></div><div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" onClick={onSelect} className="rounded-lg bg-[#63301b] px-3 py-2 text-xs font-bold text-white">إدارة المنتجات</button><button type="button" disabled={saving} onClick={() => onSave({ id: store.id, name, category, active: !store.active, sortOrder: Number(sortOrder || 0) })} className={`rounded-lg px-3 py-2 text-xs font-bold ${store.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{store.active ? "ظاهر للعميل" : "مخفي عن العميل"}</button></div></div>;
}
...
function DeliverySettingsPanel() {
  ...
  return <section className="admin-section"><div className="admin-section-heading"><div><p>صلاحية المالك والمشرف</p><h2>نسب رسوم التوصيل</h2></div><MapPinned className="h-5 w-5 text-[#63301b]" /></div><p className="settings-copy">تحسب الرسوم من قيمة المنتجات فقط، ثم تضاف إلى الإجمالي النهائي. لا تدخل رسوم التوصيل في الحد الأدنى البالغ 300 ليرة سورية جديدة. وتحدد النسبة الثالثة خصم مكافأة العميل عند استبدال كل 10 نقاط.</p><div className="grid gap-3 sm:grid-cols-2"><div><Label>نسبة رسوم طلبات منبج (%)</Label><Input inputMode="numeric" value={manbijPercent} onChange={event => setManbijPercent(event.target.value.replace(/\D/g, ""))} placeholder="15" /></div><div><Label>نسبة رسوم طلبات جرابلس (%)</Label><Input inputMode="numeric" value={jarabulusPercent} onChange={event => setJarabulusPercent(event.target.value.replace(/\D/g, ""))} placeholder="30" /></div><div><Label>نسبة مكافأة كل 10 نقاط (%)</Label><Input inputMode="numeric" value={pointsRewardPercent} onChange={event => setPointsRewardPercent(event.target.value.replace(/\D/g, ""))} placeholder="5" /></div></div><Button disabled={update.isPending || !manbijPercent || !jarabulusPercent || pointsRewardPercent === ""} onClick={() => update.mutate({ manbijPercent: Number(manbijPercent), jarabulusPercent: Number(jarabulusPercent), pointsRewardPercent: Number(pointsRewardPercent) })} className="mt-4 rounded-xl bg-[#63301b] hover:bg-[#4a2618]"><MapPinned className="h-4 w-4" /> {update.isPending ? "جارٍ الحفظ..." : "حفظ نسب التوصيل"}</Button></section>;
}
...
function PartnerRow({ partner, saving, onSave, onRemove }: { partner: { id: number; name: string; active: boolean; storeOpen: boolean; preparationMinutes: number; username: string }; saving: boolean; onSave: (values: { id: number; name: string; active: boolean; storeOpen: boolean; preparationMinutes: number }) => void; onRemove: () => void }) {
  ...
  return <div className="rounded-xl border border-slate-100 p-3"><div className="grid gap-2 md:grid-cols-[1fr_1fr_150px_auto_auto_auto]"><Input value={name} onChange={event => setName(event.target.value)} /><div className="rounded-lg bg-slate-50 px-3 py-2 text-sm font-bold text-slate-600" dir="ltr">{partner.username}</div><Input inputMode="numeric" value={preparationMinutes} onChange={event => setPreparationMinutes(event.target.value.replace(/\D/g, ""))} placeholder="دقائق" /><button type="button" disabled={saving || !name.trim()} onClick={() => save()} className="rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-[#63301b]">حفظ</button><button type="button" disabled={saving} onClick={() => save({ storeOpen: !partner.storeOpen })} className={`rounded-lg px-3 py-2 text-xs font-bold ${partner.storeOpen ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{partner.storeOpen ? "المتجر مفتوح" : "المتجر مغلق"}</button><button type="button" disabled={saving} onClick={onRemove} className="rounded-lg bg-red-50 px-3 py-2 text-red-600"><Trash2 className="h-4 w-4" /></button></div><button type="button" disabled={saving} onClick={() => save({ active: !partner.active })} className={`mt-2 rounded-lg px-3 py-1.5 text-[11px] font-bold ${partner.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{partner.active ? "الحساب نشط — اضغط للإيقاف" : "الحساب موقوف — اضغط للتفعيل"}</button></div>;
}
