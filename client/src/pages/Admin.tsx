...
type Tab = "orders" | "catalog" | "delivery" | "customers" | "employees" | "team" | "settings";
...
export default function Admin() {
  ...
  return <div dir="rtl" className="admin-app">
    ...
    <section className="admin-content">
      ...
      <div className="admin-page">{tab === "orders" ? <OrdersPanel /> : null}{tab === "catalog" ? <CatalogPanel /> : null}{tab === "delivery" ? <DeliverySettingsPanel /> : null}{tab === "customers" && isOwner ? <CustomersPanel /> : null}{tab === "employees" && isOwner ? <EmployeesPanel /> : null}{tab === "team" && isOwner ? <TeamPanel /> : null}{tab === "settings" && isOwner ? <SettingsPanel /> : null}</div>
    </section>
  </div>;
}
...
function SettingsPanel() {
  ...
  return <section className="admin-section"><div className="admin-section-heading"><div><p>حماية لوحة التحكم</p><h2>تغيير رمز PIN للمالك</h2></div><KeyRound className="h-5 w-5 text-blue-900" /></div><p className="settings-copy">لا تشارك رمز الدخول مع المشرفين. لديهم حسابات مستقلة تُنشأ من هذه اللوحة.</p><div className="pin-form"><div><Label>رمز PIN الحالي</Label><Input type="password" inputMode="numeric" value={currentPin} onChange={e => setCurrentPin(e.target.value)} /></div><div><Label>رمز PIN الجديد</Label><Input type="password" inputMode="numeric" value={newPin} onChange={e => setNewPin(e.target.value)} /></div><Button disabled={changePin.isPending} onClick={() => changePin.mutate({ currentPin, newPin })} className="rounded-xl bg-blue-900 hover:bg-blue-950"><KeyRound className="h-4 w-4" /> حفظ الرمز الجديد</Button></div></section>;
}
