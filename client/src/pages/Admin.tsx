...
import { Archive, ArrowRight, BadgeDollarSign, BadgePercent, BellRing, TicketPercent, CarFront, CheckCircle2, CircleDollarSign, ClipboardList, KeyRound, Loader2, LogOut, MapPinned, Menu, MessageCircle, PackagePlus, PackageSearch, Pencil, Phone, RefreshCw, Route, Settings2, Share2, ShieldCheck, Store, Trash2, UserPlus, UsersRound, X, XCircle } from "lucide-react";
...
export default function Admin() {
  ...
  if (!session) return <div dir="rtl" className="admin-loading"><ShieldCheck className="h-9 w-9 text-[#6f1028]" /><h1>لوحة الإدارة محمية</h1><p>افتحها عبر النقر المزدوج على شعار «لحظة» في الصفحة الرئيسية.</p><Button onClick={() => setLocation("/")} className="mt-3 rounded-xl bg-red-600 hover:bg-red-700"><ArrowRight className="h-4 w-4" /> العودة للرئيسية</Button></div>;
...
function CategoriesPanel() {
  const utils = trpc.useUtils();
  const categoriesQuery = trpc.lahza.admin.categories.list.useQuery();
  const [title, setTitle] = useState("");
  const [subtitle, setSubtitle] = useState("");
  const [sortOrder, setSortOrder] = useState("0");
  const refresh = () => { utils.lahza.admin.categories.list.invalidate(); utils.lahza.customCategories.listActive.invalidate(); };
  const create = trpc.lahza.admin.categories.create.useMutation({ onSuccess: () => { refresh(); setTitle(""); setSubtitle(""); setSortOrder("0"); toast.success("تمت إضافة القسم"); }, onError: error => toast.error(error.message) });
  const update = trpc.lahza.admin.categories.update.useMutation({ onSuccess: () => { refresh(); toast.success("تم تحديث القسم"); }, onError: error => toast.error(error.message) });
  const remove = trpc.lahza.admin.categories.remove.useMutation({ onSuccess: () => { refresh(); toast.success("تم حذف القسم الفارغ"); }, onError: error => toast.error(error.message) });
  if (categoriesQuery.isLoading) return <PanelLoading text="جارٍ تحميل الأقسام المخصصة" />;
  const categories = categoriesQuery.data ?? [];
  const saving = create.isPending || update.isPending || remove.isPending;
  return <div className="space-y-5"><section className="admin-section"><div className="admin-section-heading"><div><p>صلاحية المالك فقط</p><h2>إضافة قسم جديد</h2></div><Store className="h-5 w-5 text-red-600" /></div><p className="settings-copy">سيظهر القسم النشط للعميل ضمن «طلبك للبيت». بعد إضافته، أنشئ متجراً واختر «قسم مخصص» لربطه به.</p><div className="grid gap-3 md:grid-cols-3"><div><Label>اسم القسم</Label><Input value={title} onChange={event => setTitle(event.target.value)} placeholder="مثال: مستلزمات السيارات" /></div><div><Label>وصف قصير</Label><Input value={subtitle} onChange={event => setSubtitle(event.target.value)} placeholder="مثال: زيوت وملحقات وعناية" /></div><div><Label>ترتيب الظهور</Label><Input inputMode="numeric" value={sortOrder} onChange={event => setSortOrder(event.target.value.replace(/\D/g, ""))} /></div></div><Button disabled={create.isPending || title.trim().length < 2 || subtitle.trim().length < 2} onClick={() => create.mutate({ title: title.trim(), subtitle: subtitle.trim(), active: true, sortOrder: Number(sortOrder || 0) })} className="mt-4 rounded-xl bg-red-600 hover:bg-red-700"><Store className="h-4 w-4" /> إضافة القسم</Button></section><section className="admin-section"><div className="admin-section-heading"><div><p>إدارة الظهور</p><h2>الأقسام المضافة</h2></div><span className="admin-note">الإخفاء لا يحذف متاجر أو منتجات القسم.</span></div>{categories.length ? <div className="space-y-3">{categories.map(category => <CustomCategoryRow key={category.id} category={category} saving={saving} onSave={values => update.mutate(values)} onRemove={() => { if (confirm(`حذف القسم «${category.title}»؟ لا يمكن الحذف إذا كان مرتبطاً بمتاجر أو منتجات.`)) remove.mutate({ id: category.id }); }} />)}</div> : <Empty icon={Store} title="لا توجد أقسام مضافة بعد" text="أضف قسماً جديداً ليظهر هنا ثم اربط به متجراً من تبويب المتاجر." />}</section></div>;
}

function CustomCategoryRow({ category, saving, onSave, onRemove }: { category: { id: number; title: string; subtitle: string; active: boolean; sortOrder: number }; saving: boolean; onSave: (values: { id: number; title: string; subtitle: string; active: boolean; sortOrder: number }) => void; onRemove: () => void }) {
  const [title, setTitle] = useState(category.title);
  const [subtitle, setSubtitle] = useState(category.subtitle);
  const [sortOrder, setSortOrder] = useState(String(category.sortOrder));
  const values = (active = category.active) => ({ id: category.id, title: title.trim(), subtitle: subtitle.trim(), active, sortOrder: Number(sortOrder || 0) });
  return <article className={`rounded-2xl border p-4 ${category.active ? "border-slate-100 bg-white" : "border-amber-200 bg-amber-50/60"}`}><div className="grid gap-2 md:grid-cols-3"><div><Label>اسم القسم</Label><Input value={title} onChange={event => setTitle(event.target.value)} /></div><div><Label>الوصف</Label><Input value={subtitle} onChange={event => setSubtitle(event.target.value)} /></div><div><Label>الترتيب</Label><Input inputMode="numeric" value={sortOrder} onChange={event => setSortOrder(event.target.value.replace(/\D/g, ""))} /></div></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={saving || title.trim().length < 2 || subtitle.trim().length < 2} onClick={() => onSave(values())} className="rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-[#6f1028]">حفظ</button><button type="button" disabled={saving} onClick={() => onSave(values(!category.active))} className={`rounded-lg px-3 py-2 text-xs font-bold ${category.active ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{category.active ? "إخفاء عن العميل" : "إظهار للعميل"}</button><button type="button" disabled={saving} onClick={onRemove} className="rounded-lg bg-red-50 px-3 py-2 text-xs font-bold text-red-600">حذف القسم الفارغ</button></div></article>;
}
...
function PartnerStoreManagement() {
  ...
  const catalogQuery = trpc.lahza.catalog.list.useQuery();
  const customCategoriesQuery = trpc.lahza.admin.categories.list.useQuery();
  ...
  const updateProduct = trpc.lahza.catalog.update.useMutation({ onSuccess: () => { refresh(); toast.success("تم تحديث المنتج"); }, onError: error => toast.error(error.message) });
  const removeProduct = trpc.lahza.catalog.remove.useMutation({ onSuccess: () => { refresh(); toast.success("تم حذف المنتج"); }, onError: error => toast.error(error.message) });
  if (storesQuery.isLoading || partnersQuery.isLoading || catalogQuery.isLoading || customCategoriesQuery.isLoading) return <PanelLoading text="جارٍ تحميل المتاجر والشركاء" />;
  const stores = storesQuery.data ?? [];
  ...
  return <div className="space-y-5">
    <section className="admin-section">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        {category === "other" ? <div><Label>القسم المخصص</Label><select value={customCategoryId} onChange={event => setCustomCategoryId(event.target.value)} className="form-select"><option value="">اختر القسم</option>{customCategories.map(custom => <option key={custom.id} value={custom.id}>{custom.title}</option>)}</select></div> : null}
      ...
      <Button disabled={create.isPending || !name.trim() || (category === "other" && !customCategoryId)} onClick={() => create.mutate({ name: name.trim(), category, restaurantType, customCategoryId: category === "other" ? Number(customCategoryId) : null, partnerId: partnerId ? Number(partnerId) : null, active: true, sortOrder: Number(sortOrder || 0) })} className="mt-4 rounded-xl bg-red-600 hover:bg-red-700"><Store className="h-4 w-4" /> إضافة المتجر</Button>
    </section>
    <section className="admin-section">
    ...
    {selectedStore ? <section className="admin-section">
      ...
      <div className="grid gap-3 md:grid-cols-3"><div><Label>اسم المنتج</Label><Input value={productName} onChange={event => setProductName(event.target.value)} /></div><div><Label>السعر (ل.س جديدة)</Label><Input inputMode="numeric" value={productPrice} onChange={event => setProductPrice(event.target.value.replace(/\D/g, ""))} /></div><div className="flex items-end"><Button disabled={createProduct.isPending || !productName.trim() || productPrice === ""} onClick={() => createProduct.mutate({ name: productName.trim(), category: selectedStore.category, customCategoryId: selectedStore.customCategoryId, storeId: selectedStore.id, unit: "وحدة", price: Number(productPrice), available: true })} className="w-full rounded-xl bg-[#6f1028] hover:bg-[#54162a]"><PackagePlus className="h-4 w-4" /> إضافة المنتج</Button></div></div>
      <div className="mt-5 space-y-3">{productList.length ? productList.map(product => <CatalogRow key={product.id} product={product} onSave={values => updateProduct.mutate({ ...values, customCategoryId: selectedStore.customCategoryId, storeId: selectedStore.id })} onRemove={() => removeProduct.mutate({ id: product.id })} saving={saving} />) : <Empty icon={PackageSearch} title="لا توجد منتجات في هذا المتجر" text="أضف منتجات للمتجر ليظهر محتواها للعميل وللشريك المعيّن." />}</div>
    </section> : null}
  </div>;
}

function AssignedStoreRow({ store, partners, customCategories, selected, saving, onSelect, onSave, onRemove }: { store: { id: number; name: string; category: LahzaCategory; restaurantType: RestaurantType; customCategoryId: number | null; partnerId: number | null; active: boolean; sortOrder: number }; partners: Array<{ id: number; name: string; username: string }>; customCategories: Array<{ id: number; title: string }>; selected: boolean; saving: boolean; onSelect: () => void; onSave: (values: { id: number; name: string; category: LahzaCategory; restaurantType: RestaurantType; customCategoryId: number | null; partnerId: number | null; active: boolean; sortOrder: number }) => void; onRemove: () => void }) {
  ...
  const [customCategoryId, setCustomCategoryId] = useState(store.customCategoryId ? String(store.customCategoryId) : "");
  ...
  const values = (active = store.active) => ({ id: store.id, name: name.trim(), category, restaurantType, customCategoryId: category === "other" && customCategoryId ? Number(customCategoryId) : null, partnerId: partnerId ? Number(partnerId) : null, active, sortOrder: Number(sortOrder || 0) });
  return <article className={`rounded-2xl border p-4 ${selected ? "border-rose-300 bg-rose-50" : "border-slate-100 bg-white"}`}>
    ...
    <div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={saving || !name.trim() || (category === "other" && !customCategoryId)} onClick={() => onSave(values())} className="rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-[#6f1028]">حفظ التعيين</button><button type="button" disabled={saving} onClick={onSelect} className="rounded-lg bg-[#6f1028] px-3 py-2 text-xs font-bold text-white">إدارة المنتجات</button><button type="button" disabled={saving || (category === "other" && !customCategoryId)} onClick={() => onSave(values(!store.active))} className={`rounded-lg px-3 py-2 text-xs font-bold ${store.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{store.active ? "ظاهر للعميل" : "مخفي عن العميل"}</button><button type="button" disabled={saving} onClick={onRemove} className="rounded-lg bg-red-50 px-3 py-2 text-red-600"><Trash2 className="h-4 w-4" /></button></div>
  </article>;
}
...
function StoreRow({ store, selected, saving, onSelect, onSave, onRemove }: { store: { id: number; name: string; category: LahzaCategory; active: boolean; sortOrder: number }; selected: boolean; saving: boolean; onSelect: () => void; onSave: (values: { id: number; name: string; category: LahzaCategory; active: boolean; sortOrder: number }) => void; onRemove: () => void }) {
  ...
  const [category, setCategory] = useState<LahzaCategory>(store.category);
  const [sortOrder, setSortOrder] = useState(String(store.sortOrder));
  return <div className={`rounded-2xl border p-4 ${selected ? "border-rose-300 bg-rose-50" : "border-slate-100 bg-white"}`}><div className="grid gap-2 sm:grid-cols-[1.2fr_1fr_86px_auto_auto]"><Input value={name} onChange={event => setName(event.target.value)} /><select value={category} onChange={event => setCategory(event.target.value as LahzaCategory)} className="form-select">{(Object.keys(categoryMeta) as LahzaCategory[]).map(key => <option key={key} value={key}>{categoryMeta[key].title}</option>)}</select><Input inputMode="numeric" value={sortOrder} onChange={event => setSortOrder(event.target.value.replace(/\D/g, ""))} /><button type="button" disabled={saving || !name.trim()} onClick={() => onSave({ id: store.id, name: name.trim(), category, active: store.active, sortOrder: Number(sortOrder || 0) })} className="rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-[#6f1028]">حفظ</button><button type="button" disabled={saving} onClick={onRemove} className="rounded-lg bg-red-50 px-3 py-2 text-red-600"><Trash2 className="h-4 w-4" /></button></div><div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" onClick={onSelect} className="rounded-lg bg-[#6f1028] px-3 py-2 text-xs font-bold text-white">إدارة المنتجات</button><button type="button" disabled={saving} onClick={() => onSave({ id: store.id, name, category, active: !store.active, sortOrder: Number(sortOrder || 0) })} className={`rounded-lg px-3 py-2 text-xs font-bold ${store.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{store.active ? "ظاهر للعميل" : "مخفي عن العميل"}</button></div></div>;
}
...
function DeliverySettingsPanel() {
  ...
  const [pointsRewardPercent, setPointsRewardPercent] = useState("");
  ...
  useEffect(() => {
    ...
    setPointsRewardPercent(String(settingsQuery.data.pointsRewardPercent ?? 0));
  }, [settingsQuery.data]);
  if (settingsQuery.isLoading) return <PanelLoading text="جارٍ تحميل إعدادات التوصيل" />;
  return <section className="admin-section"><div className="admin-section-heading"><div><p>صلاحية المالك والمشرف</p><h2>نسب رسوم التوصيل</h2></div><MapPinned className="h-5 w-5 text-[#6f1028]" /></div><p className="settings-copy">تحسب الرسوم من قيمة المنتجات فقط، ثم تضاف إلى الإجمالي النهائي. لا تدخل رسوم التوصيل في الحد الأدنى البالغ 300 ليرة سورية جديدة. وتحدد النسبة الثالثة خصم مكافأة العميل عند استبدال كل 10 نقاط.</p><div className="grid gap-3 sm:grid-cols-2"><div><Label>نسبة رسوم طلبات منبج (%)</Label><Input inputMode="numeric" value={manbijPercent} onChange={event => setManbijPercent(event.target.value.replace(/\D/g, ""))} placeholder="15" /></div><div><Label>نسبة رسوم طلبات جرابلس (%)</Label><Input inputMode="numeric" value={jarabulusPercent} onChange={event => setJarabulusPercent(event.target.value.replace(/\D/g, ""))} placeholder="30" /></div><div><Label>نسبة مكافأة كل 10 نقاط (%)</Label><Input inputMode="numeric" value={pointsRewardPercent} onChange={event => setPointsRewardPercent(event.target.value.replace(/\D/g, ""))} placeholder="5" /></div></div><Button disabled={update.isPending || !manbijPercent || !jarabulusPercent || pointsRewardPercent === ""} onClick={() => update.mutate({ manbijPercent: Number(manbijPercent), jarabulusPercent: Number(jarabulusPercent), pointsRewardPercent: Number(pointsRewardPercent) })} className="mt-4 rounded-xl bg-[#6f1028] hover:bg-[#54162a]"><MapPinned className="h-4 w-4" /> {update.isPending ? "جارٍ الحفظ..." : "حفظ نسب التوصيل"}</Button></section>;
}
...
function PartnerRow({ partner, saving, onSave, onRemove }: { partner: { id: number; name: string; active: boolean; storeOpen: boolean; preparationMinutes: number; username: string }; saving: boolean; onSave: (values: { id: number; name: string; active: boolean; storeOpen: boolean; preparationMinutes: number }) => void; onRemove: () => void }) {
  ...
  return <div className="rounded-xl border border-slate-100 p-3"><div className="grid gap-2 md:grid-cols-[1fr_1fr_150px_auto_auto_auto]"><Input value={name} onChange={event => setName(event.target.value)} /><div className="rounded-lg bg-slate-50 px-3 py-2 text-sm font-bold text-slate-600" dir="ltr">{partner.username}</div><Input inputMode="numeric" value={preparationMinutes} onChange={event => setPreparationMinutes(event.target.value.replace(/\D/g, ""))} placeholder="دقائق" /><button type="button" disabled={saving || !name.trim()} onClick={() => save()} className="rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-[#6f1028]">حفظ</button><button type="button" disabled={saving} onClick={() => save({ storeOpen: !partner.storeOpen })} className={`rounded-lg px-3 py-2 text-xs font-bold ${partner.storeOpen ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{partner.storeOpen ? "المتجر مفتوح" : "المتجر مغلق"}</button><button type="button" disabled={saving} onClick={onRemove} className="rounded-lg bg-red-50 px-3 py-2 text-red-600"><Trash2 className="h-4 w-4" /></button></div><button type="button" disabled={saving} onClick={() => save({ active: !partner.active })} className={`mt-2 rounded-lg px-3 py-1.5 text-[11px] font-bold ${partner.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{partner.active ? "الحساب نشط — اضغط للإيقاف" : "الحساب موقوف — اضغط للتفعيل"}</button></div>;
}
...
function SettingsPanel() {
  ...
  return <div className="space-y-5"><section className="admin-section"><div className="admin-section-heading"><div><p>محتوى واجهة العميل</p><h2>الشريطان المتحركان</h2></div><Settings2 className="h-5 w-5 text-red-600" /></div><p className="settings-copy">يظهر النصان في شريط واحد أعلى التطبيق، ويفصل بينهما رمز نجمة تلقائياً.</p>{interfaceSettingsQuery.isLoading ? <PanelLoading text="جارٍ تحميل نصوص الشريط" /> : <div className="pin-form"><div><Label>نص الشريط الأول</Label><Input value={tickerPrimary} onChange={event => setTickerPrimary(event.target.value.slice(0, 220))} placeholder="حقق ١٠ طلبات واربح معنا هدية" /></div><div><Label>نص الشريط الثاني</Label><Input value={tickerSecondary} onChange={event => setTickerSecondary(event.target.value.slice(0, 220))} placeholder="لحظة — منبج بين يديك" /></div><Button disabled={updateInterface.isPending || tickerPrimary.trim().length < 2 || tickerSecondary.trim().length < 2} onClick={() => updateInterface.mutate({ tickerPrimary: tickerPrimary.trim(), tickerSecondary: tickerSecondary.trim() })} className="rounded-xl bg-red-600 hover:bg-red-700"><Settings2 className="h-4 w-4" /> {updateInterface.isPending ? "جارٍ الحفظ..." : "حفظ نصي الشريط"}</Button></div>}</section><section className="admin-section"><div className="admin-section-heading"><div><p>حماية لوحة التحكم</p><h2>تغيير رمز PIN للمالك</h2></div><KeyRound className="h-5 w-5 text-[#6f1028]" /></div><p className="settings-copy">لا تشارك رمز الدخول مع المشرفين. لديهم حسابات مستقلة تُنشأ من هذه اللوحة.</p><div className="pin-form"><div><Label>رمز PIN الحالي</Label><Input type="password" inputMode="numeric" value={currentPin} onChange={e => setCurrentPin(e.target.value)} /></div><div><Label>رمز PIN الجديد</Label><Input type="password" inputMode="numeric" value={newPin} onChange={e => setNewPin(e.target.value)} /></div><Button disabled={changePin.isPending} onClick={() => changePin.mutate({ currentPin, newPin })} className="rounded-xl bg-[#6f1028] hover:bg-[#54162a]"><KeyRound className="h-4 w-4" /> حفظ الرمز الجديد</Button></div></section></div>;
}

function SupportContactsPanel() {
  const utils = trpc.useUtils();
  const contactsQuery = trpc.lahza.admin.supportContacts.list.useQuery();
  const [label, setLabel] = useState("خدمة العملاء");
  const [phone, setPhone] = useState("");
  const [callEnabled, setCallEnabled] = useState(true);
  const [whatsappEnabled, setWhatsappEnabled] = useState(true);
  const refresh = () => { void contactsQuery.refetch(); void utils.lahza.support.contacts.invalidate(); };
  const create = trpc.lahza.admin.supportContacts.create.useMutation({ onSuccess: () => { setPhone(""); refresh(); toast.success("تمت إضافة جهة التواصل"); }, onError: error => toast.error(error.message) });
  if (contactsQuery.isLoading) return <PanelLoading text="جارٍ تحميل جهات التواصل" />;
  return <section className="admin-section"><div className="admin-section-heading"><div><p>محتوى واجهة العميل</p><h2>جهات التواصل</h2></div><Phone className="h-5 w-5 text-[#6f1028]" /></div><p className="settings-copy">أضف أي عدد من الأرقام، ثم حدد لكل رقم إن كان يظهر للاتصال أو واتساب أو كليهما. تظهر الجهات المفعلة للعميل في «تواصل معنا» وصفحة تأكيد الطلب.</p><div className="rounded-2xl border border-rose-100 bg-[#fff9f6] p-4"><div className="grid gap-3 md:grid-cols-2"><div><Label>اسم الجهة</Label><Input value={label} onChange={event => setLabel(event.target.value.slice(0, 80))} placeholder="مثال: خدمة العملاء" /></div><div><Label>رقم الهاتف السوري</Label><div className="phone-entry" dir="ltr"><span>+963</span><Input inputMode="numeric" value={phone} onChange={event => setPhone(event.target.value.replace(/\D/g, "").replace(/^963/, "").slice(0, 9))} placeholder="9XXXXXXXX" /></div></div></div><div className="mt-3 flex flex-wrap gap-4 text-xs font-bold text-slate-600"><label className="inline-flex items-center gap-2"><input type="checkbox" checked={callEnabled} onChange={event => setCallEnabled(event.target.checked)} /> اتصال هاتفي</label><label className="inline-flex items-center gap-2"><input type="checkbox" checked={whatsappEnabled} onChange={event => setWhatsappEnabled(event.target.checked)} /> واتساب</label></div><Button disabled={create.isPending || label.trim().length < 2 || !/^9\d{8}$/.test(phone) || (!callEnabled && !whatsappEnabled)} onClick={() => create.mutate({ label: label.trim(), phone: `+963${phone}`, callEnabled, whatsappEnabled, active: true, sortOrder: (contactsQuery.data?.length ?? 0) + 1 })} className="mt-4 rounded-xl bg-[#6f1028] hover:bg-[#54162a]"><UserPlus className="h-4 w-4" /> {create.isPending ? "جارٍ الإضافة..." : "إضافة جهة تواصل"}</Button></div><div className="mt-4 space-y-3">{contactsQuery.data?.length ? contactsQuery.data.map(contact => <SupportContactEditor key={contact.id} contact={contact as AdminSupportContact} onChanged={refresh} />) : <div className="rounded-2xl border border-dashed border-slate-200 p-5 text-center text-sm font-bold text-slate-500">لم تُضف أي جهة تواصل بعد.</div>}</div></section>;
}

function SupportContactEditor({ contact, onChanged }: { contact: AdminSupportContact; onChanged: () => void }) {
  const [label, setLabel] = useState(contact.label);
  const [phone, setPhone] = useState(contact.phone.replace(/^\+963/, ""));
  const [callEnabled, setCallEnabled] = useState(contact.callEnabled);
  const [whatsappEnabled, setWhatsappEnabled] = useState(contact.whatsappEnabled);
  const [active, setActive] = useState(contact.active);
  const [sortOrder, setSortOrder] = useState(String(contact.sortOrder));
  useEffect(() => { setLabel(contact.label); setPhone(contact.phone.replace(/^\+963/, "")); setCallEnabled(contact.callEnabled); setWhatsappEnabled(contact.whatsappEnabled); setActive(contact.active); setSortOrder(String(contact.sortOrder)); }, [contact]);
  const update = trpc.lahza.admin.supportContacts.update.useMutation({ onSuccess: () => { onChanged(); toast.success("تم حفظ جهة التواصل"); }, onError: error => toast.error(error.message) });
  const remove = trpc.lahza.admin.supportContacts.remove.useMutation({ onSuccess: () => { onChanged(); toast.success("تم حذف جهة التواصل"); }, onError: error => toast.error(error.message) });
  const canSave = label.trim().length >= 2 && /^9\d{8}$/.test(phone) && (callEnabled || whatsappEnabled) && Number.isInteger(Number(sortOrder));
  return <article className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm"><div className="grid gap-3 md:grid-cols-[1fr_1fr_8rem]"><div><Label>اسم الجهة</Label><Input value={label} onChange={event => setLabel(event.target.value.slice(0, 80))} /></div><div><Label>الرقم</Label><div className="phone-entry" dir="ltr"><span>+963</span><Input inputMode="numeric" value={phone} onChange={event => setPhone(event.target.value.replace(/\D/g, "").replace(/^963/, "").slice(0, 9))} /></div></div><div><Label>الترتيب</Label><Input inputMode="numeric" value={sortOrder} onChange={event => setSortOrder(event.target.value.replace(/\D/g, "").slice(0, 5))} /></div></div><div className="mt-3 flex flex-wrap items-center gap-4 text-xs font-bold text-slate-600"><label className="inline-flex items-center gap-2"><input type="checkbox" checked={callEnabled} onChange={event => setCallEnabled(event.target.checked)} /> اتصال</label><label className="inline-flex items-center gap-2"><input type="checkbox" checked={whatsappEnabled} onChange={event => setWhatsappEnabled(event.target.checked)} /> <MessageCircle className="h-3.5 w-3.5 text-emerald-600" /> واتساب</label><label className="inline-flex items-center gap-2"><input type="checkbox" checked={active} onChange={event => setActive(event.target.checked)} /> ظاهر للعميل</label><div className="mr-auto flex gap-2"><Button disabled={update.isPending || !canSave} onClick={() => update.mutate({ id: contact.id, label: label.trim(), phone: `+963${phone}`, callEnabled, whatsappEnabled, active, sortOrder: Number(sortOrder) })} size="sm" className="rounded-lg bg-[#6f1028] hover:bg-[#54162a]">حفظ</Button><Button disabled={remove.isPending} onClick={() => remove.mutate({ id: contact.id })} size="sm" variant="outline" className="rounded-lg border-red-100 text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4" /></Button></div></div></article>;
}
