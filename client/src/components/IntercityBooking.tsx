...
export function IntercityBooking({ onBack, onChooseTrip, isStaticDemo }: { onBack: () => void; onChooseTrip: (trip: IntercityTripSelection) => void; isStaticDemo: boolean }) {
  ...
  return <>
    ...
    <section className="app-shell space-y-5 pb-12"><div className="intercity-panel"><span className="intercity-panel-icon"><Route /></span><p className="section-eyebrow">الحجوزات الحالية</p><h2>اختر الحجز المناسب لك</h2><div className="mt-5 grid gap-3">{tripsQuery.isLoading && !isStaticDemo ? <p className="text-sm text-slate-500">جارٍ تحميل الحجوزات...</p> : trips.length ? trips.map(trip => { const open = trip.status === "open" && trip.active && trip.capacity > trip.reservedCount; return <button key={trip.id} onClick={() => setSelectedTripId(trip.id)} className={`rounded-2xl border p-4 text-right ${selectedTrip?.id === trip.id ? "border-[#7a1430] bg-rose-50 ring-2 ring-rose-100" : "border-slate-200 bg-white"}`}><div className="flex items-start justify-between gap-3"><div><strong className="text-base text-[#54162a]">{trip.title}</strong><span className="mt-1 block text-xs text-slate-500">{trip.bookingCloseLabel} · {trip.arrivalLabel}</span></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${open ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{open ? `${Math.max(0, trip.capacity - trip.reservedCount)} أماكن متبقية` : "الحجز مغلق"}</span></div></button>; }) : <p className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">لا توجد حجوزات متاحة حالياً. تابع لحظة لمعرفة موعد الحجز التالي.</p>}</div></div>
      <div className="rounded-3xl border border-rose-100 bg-rose-50 p-5"><div className="flex gap-3"><CheckCircle2 className="h-6 w-6 shrink-0 text-[#6f1028]" /><div><h2 className="font-black text-[#54162a]">ماذا بعد اختيار الحجز؟</h2><p className="mt-2 text-sm leading-7 text-slate-600">ستفتح لك أقسام طلبات منبج نفسها. أضف أي منتجات إلى السلة ثم أرسل الطلب، وسيُسجل ضمن الحجز الذي اخترته.</p></div></div><Button disabled={!isBookable} onClick={continueToManbijCatalog} className="mt-5 h-12 w-full rounded-2xl bg-[#54162a] hover:bg-[#6f1028]">الانتقال إلى أقسام منبج</Button></div>
    </section>
  </>;
}
